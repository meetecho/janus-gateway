...
int janus_rabbitmqevh_init(const char *config_path) {
	...
	/* Read configuration */
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.yaml", config_path, JANUS_RABBITMQEVH_PACKAGE);
	...
	item = janus_config_get_item(config_general, "grouping");
	if(item && item->value)
