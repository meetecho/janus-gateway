 ...
 * \brief  Janus NNGEventHandler plugin
 * \details  This is a trivial NNG event handler plugin for Janus
 *
...
/* Plugin information */
#define JANUS_NNGEVH_VERSION			1
#define JANUS_NNGEVH_VERSION_STRING		"0.0.1"
#define JANUS_NNGEVH_DESCRIPTION		"This is a trivial NNG event handler plugin for Janus."
#define JANUS_NNGEVH_NAME				"JANUS NNGEventHandler plugin"
#define JANUS_NNGEVH_AUTHOR				"Meetecho s.r.l."
#define JANUS_NNGEVH_PACKAGE			"janus.eventhandler.nngevh"

/* Plugin methods */
janus_eventhandler *create(void);
int janus_nngevh_init(const char *config_path);
void janus_nngevh_destroy(void);
int janus_nngevh_get_api_compatibility(void);
int janus_nngevh_get_version(void);
const char *janus_nngevh_get_version_string(void);
const char *janus_nngevh_get_description(void);
const char *janus_nngevh_get_name(void);
const char *janus_nngevh_get_author(void);
const char *janus_nngevh_get_package(void);
void janus_nngevh_incoming_event(json_t *event);
json_t *janus_nngevh_handle_request(json_t *request);

/* Event handler setup */
static janus_eventhandler janus_nngevh =
	JANUS_EVENTHANDLER_INIT (
		.init = janus_nngevh_init,
		.destroy = janus_nngevh_destroy,

		.get_api_compatibility = janus_nngevh_get_api_compatibility,
		.get_version = janus_nngevh_get_version,
		.get_version_string = janus_nngevh_get_version_string,
		.get_description = janus_nngevh_get_description,
		.get_name = janus_nngevh_get_name,
		.get_author = janus_nngevh_get_author,
		.get_package = janus_nngevh_get_package,

		.incoming_event = janus_nngevh_incoming_event,
		.handle_request = janus_nngevh_handle_request,
...
static void *janus_nngevh_thread(void *data);
static void *janus_nngevh_handler(void *data);

/* Queue of events to handle */
static GAsyncQueue *events = NULL, *nfd_queue = NULL;
static gboolean group_events = TRUE;
static json_t exit_event;
static void janus_nngevh_event_free(json_t *event) {
...
/* NNG stuff */
static nng_socket nfd = NNG_SOCKET_INITIALIZER, write_nfd[2];
...
#define JANUS_NNGEVH_ERROR_INVALID_REQUEST		411
#define JANUS_NNGEVH_ERROR_MISSING_ELEMENT		412
#define JANUS_NNGEVH_ERROR_INVALID_ELEMENT		413
#define JANUS_NNGEVH_ERROR_UNKNOWN_ERROR		499
...
int janus_nngevh_init(const char *config_path) {
	...
	/* Read configuration */
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.jcfg", config_path, JANUS_NNGEVH_PACKAGE);
	JANUS_LOG(LOG_VERB, "Configuration file: %s\n", filename);
	janus_config *config = janus_config_parse(filename);
	if(config == NULL) {
		JANUS_LOG(LOG_WARN, "Couldn't find .jcfg configuration file (%s), trying .cfg\n", JANUS_NNGEVH_PACKAGE);
		g_snprintf(filename, 255, "%s/%s.cfg", config_path, JANUS_NNGEVH_PACKAGE);
		JANUS_LOG(LOG_VERB, "Configuration file: %s\n", filename);
	...
	if(!item || !item->value || !janus_is_true(item->value)) {
		JANUS_LOG(LOG_WARN, "NNG event handler disabled\n");
		goto error;
	}
	...
	if(item && item->value)
		janus_events_edit_events_mask(item->value, &janus_nngevh.events_mask);
	...
	if((res = nng_listen(write_nfd[0], "inproc://janusevh", NULL, 0)) != 0) {
		JANUS_LOG(LOG_ERR, "Error configuring internal NNG pipeline... %d (%s)\n", res, nng_strerror(res));
		goto error;
	}
	if((res = nng_dial(write_nfd[1], "inproc://janusevh", NULL, 0)) != 0) {
		JANUS_LOG(LOG_ERR, "Error configuring internal NNG pipeline...%d (%s)\n", res, nng_strerror(res));
		goto error;
	}
	/* Handle the NNG configuration */
	...
	if(mode == NULL)
		mode = "connect";
	if((res = nng_pub0_open(&nfd)) != 0) {
		JANUS_LOG(LOG_ERR, "Error creating NNG event handler socket: %d (%s)\n", res, nng_strerror(res));
		goto error;
	}
	if(!strcasecmp(mode, "listen")) {
		/* Listen this address */
		if((res = nng_listen(nfd, address, NULL, 0)) != 0) {
			JANUS_LOG(LOG_ERR, "Error listening NNG event handler socket to address '%s': %d (%s)\n",
		...
		if((res = nng_dial(nfd, address, NULL, 0)) != 0) {
			JANUS_LOG(LOG_ERR, "Error dialing NNG event handler socket to address '%s': %d (%s)\n",
				address, res, nng_strerror(res));
			goto error;
		}
	} else {
	...
	events = g_async_queue_new_full((GDestroyNotify) janus_nngevh_event_free);
	nfd_queue = g_async_queue_new_full((GDestroyNotify) g_free);
	g_atomic_int_set(&initialized, 1);

	/* Start the NNG and event handler threads */
	GError *error = NULL;
	handler_thread = g_thread_try_new("janus nngevh thread", janus_nngevh_thread, NULL, &error);
	if(error != NULL) {
		g_atomic_int_set(&initialized, 0);
		JANUS_LOG(LOG_FATAL, "Got error %d (%s) trying to launch the NNGEventHandler loop thread...\n",
			error->code, error->message ? error->message : "??");
		g_error_free(error);
		goto error;
	}
	error = NULL;
	handler_thread = g_thread_try_new("janus nngevh handler", janus_nngevh_handler, NULL, &error);
	if(error != NULL) {
		g_atomic_int_set(&initialized, 0);
		JANUS_LOG(LOG_FATAL, "Got error %d (%s) trying to launch the NNGEventHandler handler thread...\n",
			error->code, error->message ? error->message : "??");
		g_error_free(error);
		goto error;
	}
	...
	/* Done */
	JANUS_LOG(LOG_INFO, "Setup of NNG event handler completed\n");
	goto done;
...
int janus_nngevh_get_version(void) {
	return JANUS_NNGEVH_VERSION;
}

const char *janus_nngevh_get_version_string(void) {
	return JANUS_NNGEVH_VERSION_STRING;
}

const char *janus_nngevh_get_description(void) {
	return JANUS_NNGEVH_DESCRIPTION;
}

const char *janus_nngevh_get_name(void) {
	return JANUS_NNGEVH_NAME;
}

const char *janus_nngevh_get_author(void) {
	return JANUS_NNGEVH_AUTHOR;
}

const char *janus_nngevh_get_package(void) {
	return JANUS_NNGEVH_PACKAGE;
}
...
json_t *janus_nngevh_handle_request(json_t *request) {
	...
	JANUS_VALIDATE_JSON_OBJECT(request, request_parameters,
		...
		JANUS_NNGEVH_ERROR_MISSING_ELEMENT, JANUS_NNGEVH_ERROR_INVALID_ELEMENT);
	if(error_code != 0)
	...
	if(!strcasecmp(request_text, "tweak")) {
		...
		if(json_object_get(request, "events"))
			janus_events_edit_events_mask(json_string_value(json_object_get(request, "events")), &janus_nngevh.events_mask);
		/* Grouping */
		if(json_object_get(request, "grouping"))
			group_events = json_is_true(json_object_get(request, "grouping"));
	} else {
		JANUS_LOG(LOG_VERB, "Unknown request '%s'\n", request_text);
		error_code = JANUS_NNGEVH_ERROR_INVALID_REQUEST;
...
static void *janus_nngevh_handler(void *data) {
	JANUS_LOG(LOG_VERB, "Joining NNGEventHandler handler thread\n");
	...
	JANUS_LOG(LOG_VERB, "Leaving NNGEventHandler handler thread\n");
	return NULL;
}

/* Thread */
void *janus_nngevh_thread(void *data) {
	JANUS_LOG(LOG_VERB, "Joining NNGEventHandler loop thread\n");

	int fds = 0;
	struct pollfd poll_nfds[2];
	char buffer[1];

	int pwnfd = -1, pnfd = -1, res = 0;
	if((res = nng_getopt_int(write_nfd[0], NNG_OPT_RECVFD, &pwnfd)) != 0) {
		JANUS_LOG(LOG_WARN, "Error getting NNG pipeline descriptor... %d (%s)\n", res, nng_strerror(res));
	}
	//~ if(nng_socket_id(nfd) != -1 && (res = nng_getopt_int(nfd, NNG_OPT_SENDFD, &pnfd)) != 0) {
		//~ JANUS_LOG(LOG_WARN, "Error getting NNG event handler descriptor... id=%d, err=%d (%s)\n",
			//~ nng_socket_id(nfd), res, nng_strerror(res));
	//~ }
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		fds++;
		//~ /* Publisher socket */
		//~ if(nng_socket_id(nfd) != -1 && g_async_queue_length(nfd_queue) > 0) {
			//~ JANUS_LOG(LOG_WARN, "Polling publisher socket: %d\n", pnfd);
			//~ poll_nfds[fds].fd = pnfd;
			//~ poll_nfds[fds].events |= POLLOUT;
			//~ fds++;
		//~ }
		/* Start polling */
		int res = poll(poll_nfds, fds, -1);
		JANUS_LOG(LOG_WARN, "%d\n", res);
		if(res == 0)
			continue;
		if(res < 0) {
			if(errno == EINTR) {
		...
		for(i=0; i<fds; i++) {
			...
			//~ if(poll_nfds[i].revents & POLLOUT) {
				...
				//~ if(poll_nfds[i].fd == pnfd) {
					//~ char *payload = NULL;
					//~ while((payload = g_async_queue_try_pop(nfd_queue)) != NULL) {
						//~ JANUS_LOG(LOG_WARN, "Writing\n");
						//~ int res = nng_send(nfd, (void *)payload, strlen(payload), 0);
						//~ /* FIXME Should we check if sent everything? */
						//~ JANUS_LOG(LOG_WARN, "Written %d/%zu bytes on %d\n", res, strlen(payload), poll_nfds[i].fd);
						//~ g_free(payload);
					//~ }
				//~ }
			//~ }
			if(poll_nfds[i].revents & POLLIN) {
				if(poll_nfds[i].fd == pwnfd) {
					...
					JANUS_LOG(LOG_WARN, "Reading\n");
					(void)nng_recv(write_nfd[0], buffer, &buflen, 0);
					JANUS_LOG(LOG_WARN, "  -- Read!\n");
					char *payload = NULL;
					while((payload = g_async_queue_try_pop(nfd_queue)) != NULL) {
						JANUS_LOG(LOG_WARN, "Writing\n");
						int res = nng_send(nfd, (void *)payload, strlen(payload), 0);
						if(res < 0) {
							JANUS_LOG(LOG_WARN, "Error sending message: %d (%s)\n", res, nng_strerror(res));
						}
						g_free(payload);
					}
				}
	...
	/* Done */
	JANUS_LOG(LOG_VERB, "Leaving NNGEventHandler loop thread\n");
	return NULL;
}
