...
shared void janus_ice_init(gboolean ice_lite, gboolean ice_tcp, gboolean ipv6, uint16_t rtp_min_port, uint16_t rtp_max_port);
/*! \brief ICE stuff de-initialization */
shared void janus_ice_deinit(void);
...
shared int janus_ice_set_stun_server(gchar *stun_server, uint16_t stun_port);
 ...
 * @returns 0 in case of success, a negative integer on errors */
shared int janus_ice_set_turn_server(gchar *turn_server, uint16_t turn_port, gchar *turn_type, gchar *turn_user, gchar *turn_pwd);
 ...
 * @returns 0 in case of success, a negative integer on errors */
shared int janus_ice_set_turn_rest_api(gchar *api_server, gchar *api_key, gchar *api_method);
...
shared void janus_ice_enforce_interface(const char *ip);
...
shared gboolean janus_ice_is_enforced(const char *ip);
...
shared void janus_ice_ignore_interface(const char *ip);
...
shared gboolean janus_ice_is_ignored(const char *ip);
...
shared gboolean janus_ice_is_ice_lite_enabled(void);
/*! \brief Method to check whether ICE-TCP support is enabled/supported or not (still WIP)
 * @returns true if ICE-TCP support is enabled/supported, false otherwise */
shared gboolean janus_ice_is_ice_tcp_enabled(void);
...
shared void janus_ice_force_rtcpmux(gboolean forced);
...
shared gint janus_ice_get_rtcpmux_blackhole_port(void);
...
shared void janus_set_max_nack_queue(uint mnq);
...
shared uint janus_get_max_nack_queue(void);
...
shared const gchar *janus_get_ice_state_name(gint state);
...
shared void janus_ice_stats_reset(janus_ice_stats *stats);
...
shared void janus_ice_notify_hangup(janus_ice_handle *handle, const char *reason);
...
shared gboolean janus_plugin_session_is_alive(janus_plugin_session *plugin_session);
...
shared janus_ice_trickle *janus_ice_trickle_new(janus_ice_handle *handle, const char *transaction, json_t *candidate);
...
shared gint janus_ice_trickle_parse(janus_ice_handle *handle, json_t *candidate, const char **error);
/*! \brief Helper method to destroy a janus_ice_trickle instance
 * @param[in] trickle The janus_ice_trickle instance to destroy */
shared void janus_ice_trickle_destroy(janus_ice_trickle *trickle);
///@}
...
shared janus_ice_handle *janus_ice_handle_create(void *gateway_session);
 ...
 * @returns The created Janus ICE handle if successful, NULL otherwise */
shared janus_ice_handle *janus_ice_handle_find(void *gateway_session, guint64 handle_id);
...
shared gint janus_ice_handle_attach_plugin(void *gateway_session, guint64 handle_id, janus_plugin *plugin);
/*! \brief Method to destroy a Janus ICE handle
 * @param[in] gateway_session The gateway/peer session this ICE handle belongs to
 * @param[in] handle_id The Janus ICE handle ID to destroy
 * @returns 0 in case of success, a negative integer otherwise */
shared gint janus_ice_handle_destroy(void *gateway_session, guint64 handle_id);
...
shared void janus_ice_free(janus_ice_handle *handle);
...
shared void janus_ice_webrtc_hangup(janus_ice_handle *handle);
...
shared void janus_ice_webrtc_free(janus_ice_handle *handle);
...
shared void janus_ice_stream_free(GHashTable *container, janus_ice_stream *stream);
...
shared void janus_ice_component_free(GHashTable *container, janus_ice_component *component);
///@}
...
shared void janus_ice_cb_candidate_gathering_done (NiceAgent *agent, guint stream_id, gpointer ice);
 ...
 * @param[in] ice Opaque pointer to the Janus ICE handle associated with the libnice ICE agent */
shared void janus_ice_cb_component_state_changed (NiceAgent *agent, guint stream_id, guint component_id, guint state, gpointer ice);
...
shared void janus_ice_cb_new_selected_pair (NiceAgent *agent, guint stream_id, guint component_id, gchar *local, gchar *remote, gpointer ice);
#else
shared void janus_ice_cb_new_selected_pair (NiceAgent *agent, guint stream_id, guint component_id, NiceCandidate *local, NiceCandidate *remote, gpointer ice);
#endif
 ...
 * @param[in] ice Opaque pointer to the Janus ICE handle associated with the libnice ICE agent */
#ifndef HAVE_LIBNICE_TCP
shared void janus_ice_cb_new_remote_candidate (NiceAgent *agent, guint stream_id, guint component_id, gchar *candidate, gpointer ice);
#else
shared void janus_ice_cb_new_remote_candidate (NiceAgent *agent, NiceCandidate *candidate, gpointer ice);
#endif
 ...
 * @param[in] ice Opaque pointer to the Janus ICE handle associated with the libnice ICE agent */
shared void janus_ice_cb_nice_recv (NiceAgent *agent, guint stream_id, guint component_id, guint len, gchar *buf, gpointer ice);
...
shared void janus_ice_relay_rtp(janus_ice_handle *handle, int video, char *buf, int len);
 ...
 * @param[in] len The buffer lenght */
shared void janus_ice_relay_rtcp(janus_ice_handle *handle, int video, char *buf, int len);
/*! \brief Gateway SCTP/DataChannel callback, called when a plugin has data to send to a peer
 * @param[in] handle The Janus ICE handle associated with the peer
 * @param[in] buf The message data (buffer)
 * @param[in] len The buffer lenght */
shared void janus_ice_relay_data(janus_ice_handle *handle, char *buf, int len);
...
shared void janus_ice_incoming_data(janus_ice_handle *handle, char *buffer, int length);
///@}
...
shared void *janus_ice_thread(void *data);
/*! \brief Janus ICE thread for sending outgoing packets */
shared void *janus_ice_send_thread(void *data);
 ...
 * @returns 0 in case of success, a negative integer otherwise */
shared int janus_ice_setup_local(janus_ice_handle *handle, int offer, int audio, int video, int data, int bundle, int rtcpmux, int trickle);
...
shared void janus_ice_candidates_to_sdp(janus_ice_handle *handle, char *sdp, guint stream_id, guint component_id);
/*! \brief Method to handle remote candidates and start the connectivity checks
 * @param[in] handle The Janus ICE handle this method refers to
 * @param[in] stream_id The stream ID of the candidate to add to the SDP
 * @param[in] component_id The component ID of the candidate to add to the SDP */
shared void janus_ice_setup_remote_candidates(janus_ice_handle *handle, guint stream_id, guint component_id);
...
shared void janus_ice_dtls_handshake_done(janus_ice_handle *handle, janus_ice_component *component);
///@}
