 ...
 * \ref plugins
 *
 * \page textroom Janus DataRoom documentation
 ...
 *
 * \section textroomapi Data Room API
 *
 * All DataRoom API requests are addressed by a \c textroom named property,
 ...
 * feature in the DataRoom plugin, and must be formatted like this:
 *
\verbatim
{
...
#define JANUS_TEXTROOM_NAME				"JANUS DataRoom plugin"
...
int janus_textroom_init(janus_callbacks *callback, const char *config_path) {
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			...
			JANUS_LOG(LOG_VERB, "Created DataRoom: %s (%s, %s, secret: %s, pin: %s, history: %"SCNu16" messages)\n",
...
void janus_textroom_incoming_data(janus_plugin_session *handle, janus_plugin_data *packet) {
	if(handle == NULL || handle->stopped || g_atomic_int_get(&stopping) || !g_atomic_int_get(&initialized))
		return;
	if(packet->binary) {
		/* We don't support binary data in the DataRoom plugin, it has to be text */
...
janus_plugin_result *janus_textroom_handle_incoming_request(janus_plugin_session *handle, char *text, json_t *json, gboolean internal) {
	...
	if(!strcasecmp(request_text, "message")) {
		...
		JANUS_LOG(LOG_VERB, "Attempt to send a DataRoom announcement\n");
