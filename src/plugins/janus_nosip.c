		...
		"forwarders" : [
			{
				...
				"mindex" : "<m-line of the forwarded stream>",
...
static janus_rtp_forwarder *janus_nosip_rtp_forwarder_add_helper(janus_nosip_session *session, int mindex, const char *type,
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(!strcasecmp(request_text, "generate") || !strcasecmp(request_text, "process")) {
			...
			for(i=0; i<json_array_size(streams); i++) {
				...
				janus_rtp_forwarder *f = janus_nosip_rtp_forwarder_add_helper(session, mindex, type,
...
static janus_rtp_forwarder *janus_nosip_rtp_forwarder_add_helper(janus_nosip_session *session, int mindex, const char *type,
	...
	/* Done */
	JANUS_LOG(LOG_VERB, "[NoSIP-%p][#%d] Added %s' %s rtp_forward: %s:%d stream_id: %"SCNu32"\n",
		session, mindex, is_peer ? "peer" : "user", is_video ? "video" : "audio", host, port, rf->stream_id);
	return rf;
}
