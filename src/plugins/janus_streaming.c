...
#ifdef HAVE_IMQUIC
	{"roqflow", JANUS_JSON_INTEGER, JANUS_JSON_PARAM_POSITIVE},
#endif
};
...
#ifdef HAVE_IMQUIC
static struct janus_json_parameter roq_create_parameters[] = {
...
#ifdef HAVE_IMQUIC
	struct janus_streaming_roq_server *srs;
#endif
	volatile gint destroyed;
	janus_mutex mutex;
	janus_refcount ref;
} janus_streaming_mountpoint;
static GHashTable *mountpoints = NULL, *mountpoints_temp = NULL;
static janus_mutex mountpoints_mutex = JANUS_MUTEX_INITIALIZER;
static char *admin_key = NULL;
static void janus_streaming_process_packet(janus_streaming_mountpoint *mountpoint, char *name,
	janus_streaming_rtp_source_stream *stream, gboolean rtcp, char *buffer, int bytes, int index);
...
#ifdef HAVE_IMQUIC
typedef struct janus_streaming_roq_server {
	char *name;
	janus_roq_server *server;
	GList *mountpoints;
	volatile gint destroyed;
	janus_mutex mutex;
	janus_refcount ref;
} janus_streaming_roq_server;
static void janus_streaming_roq_server_destroy(janus_streaming_roq_server *roq_server) {
	if(roq_server && g_atomic_int_compare_and_exchange(&roq_server->destroyed, 0, 1)) {
		janus_roq_server_destroy(roq_server->server);
		janus_refcount_decrease(&roq_server->ref);
	}
}
...
static GHashTable *roq_servers = NULL;
static janus_mutex roq_mutex = JANUS_MUTEX_INITIALIZER;

static void janus_streaming_roq_new_client(struct janus_roq_server *rs, imquic_connection *conn) {
	janus_streaming_roq_server *srs = (janus_streaming_roq_server *)rs->metadata;
	if(!srs || g_atomic_int_get(&srs->destroyed))
		return;
	JANUS_LOG(LOG_INFO, "[%s] New RoQ connection\n", srs->name);
	/* TODO */
}
static void janus_streaming_roq_incoming_rtp(struct janus_roq_server *rs, imquic_connection *conn,
		imquic_roq_multiplexing multiplexing, uint64_t flow_id, char *buffer, int len) {
	janus_streaming_roq_server *srs = (janus_streaming_roq_server *)rs->metadata;
	if(!srs || g_atomic_int_get(&srs->destroyed))
		return;
	JANUS_LOG(LOG_HUGE, "[%s] Incoming RTP packet\n", srs->name);
	janus_mutex_lock(&srs->mutex);
	janus_streaming_mountpoint *mp = NULL;
	janus_streaming_rtp_source *source = NULL;
	janus_streaming_rtp_source_stream *stream = NULL;
	GList *temp = srs->mountpoints;
	while(temp) {
		mp = (janus_streaming_mountpoint *)temp->data;
		if(mp == NULL || mp->streaming_source != janus_streaming_source_rtp ||
				g_atomic_int_get(&mp->destroyed)) {
			temp = temp->next;
			continue;
		}
		source = mp->source;
		GList *m = source->media;
		while(m) {
			stream = (janus_streaming_rtp_source_stream *)m->data;
			if(stream->roq_flow_id != flow_id) {
				m = m->next;
				continue;
			}
			janus_streaming_process_packet(mp, srs->name, stream,
				janus_is_rtcp(buffer, len), buffer, len, 0);
			m = m->next;
		}
		temp = temp->next;
	}
	janus_mutex_unlock(&srs->mutex);
}
static void janus_streaming_roq_client_gone(struct janus_roq_server *rs, imquic_connection *conn) {
	janus_streaming_roq_server *srs = (janus_streaming_roq_server *)rs->metadata;
	if(!srs || g_atomic_int_get(&srs->destroyed))
		return;
...
janus_streaming_rtp_source_stream *janus_streaming_create_rtp_source_stream(
		const char *name, int mindex,
		gboolean roq, uint64_t flow_id,	/* Ignored when RoQ isn't available */
		const char *type, const char *mid, const char *label, const char *msid,
...
#ifdef HAVE_IMQUIC
		janus_streaming_roq_server *roq_server,
#endif
...
#ifdef HAVE_IMQUIC
		if(mountpoint->srs != NULL) {
			janus_mutex_lock(&mountpoint->srs->mutex);
			mountpoint->srs->mountpoints = g_list_remove(mountpoint->srs->mountpoints, mountpoint);
			janus_mutex_unlock(&mountpoint->srs->mutex);
		}
#endif
	}
...
int janus_streaming_init(janus_callbacks *callback, const char *config_path) {
...
#ifdef HAVE_IMQUIC
	/* Check if there are RoQ servers to pre-create */
	roq_servers = g_hash_table_new_full(g_str_hash, g_str_equal,
		(GDestroyNotify)g_free, (GDestroyNotify)janus_streaming_roq_server_destroy);
	if(config != NULL) {
		janus_config_category *config_roq = janus_config_get_create(config, NULL, janus_config_type_category, "roq");
		GList *clist = janus_config_get_categories(config, config_roq), *cl = clist;
		while(cl != NULL) {
			...
			JANUS_LOG(LOG_INFO, "Adding RoQ server '%s'\n", cat->name);
			if(g_hash_table_lookup(roq_servers, cat->name) != NULL) {
				JANUS_LOG(LOG_WARN, "  -- RoQ server '%s' already exists, skipping\n", cat->name);
				cl = cl->next;
				continue;
			}
			/* Create a new RoQ server */
			janus_config_item *ip = janus_config_get(config, cat, janus_config_type_item, "ip");
			if(ip == NULL || ip->value == NULL) {
				JANUS_LOG(LOG_WARN, "  -- Invalid IP, skipping RoQ server '%s'...\n", cat->name);
				cl = cl->next;
				continue;
			}
			janus_config_item *port = janus_config_get(config, cat, janus_config_type_item, "port");
			uint16_t port_num = port->value ? atoi(port->value) : 0;
			if(port_num == 0) {
				JANUS_LOG(LOG_WARN, "  -- Invalid port, skipping RoQ server '%s'...\n", cat->name);
				cl = cl->next;
				continue;
			}
			...
			janus_streaming_roq_server *srs = g_malloc0(sizeof(janus_streaming_roq_server));
			srs->name = g_strdup(cat->name);
			srs->server = rs;
			rs->metadata = srs;
			janus_mutex_init(&srs->mutex);
			janus_refcount_init(&srs->ref, janus_streaming_roq_server_free);
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			janus_config_category *cat = (janus_config_category *)cl->data;
			if(cat->name == NULL || !strcasecmp(cat->name, "general") || !strcasecmp(cat->name, "roq")) {
				cl = cl->next;
				continue;
			}
			...
			if(!strcasecmp(type->value, "rtp") || !strcasecmp(type->value, "roq")) {
				...
				/* Check if this is an RTP over QUIC mountpoint, or a "standard" one */
				gboolean roq = !strcasecmp(type->value, "roq");
				janus_streaming_roq_server *srs = NULL;
				if(roq) {
#ifndef HAVE_IMQUIC
					JANUS_LOG(LOG_ERR, "Can't add 'roq' mountpoint '%s', RoQ support unavailable...\n", cat->name);
					cl = cl->next;
					continue;
#else
					if(media == NULL) {
						JANUS_LOG(LOG_ERR, "Can't add 'roq' mountpoint '%s', missing media array...\n", cat->name);
						cl = cl->next;
						continue;
					}
					janus_config_item *roqname = janus_config_get(config, cat, janus_config_type_item, "roq");
					if(!roqname || !roqname->value) {
						JANUS_LOG(LOG_ERR, "Can't add 'roq' mountpoint '%s', missing RoQ server name...\n", cat->name);
						cl = cl->next;
						continue;
					}
					janus_mutex_lock(&roq_mutex);
					srs = g_hash_table_lookup(roq_servers, roqname->value);
					janus_mutex_unlock(&roq_mutex);
					if(srs == NULL) {
						JANUS_LOG(LOG_ERR, "Can't add 'roq' mountpoint '%s', invalid RoQ server...\n", cat->name);
						cl = cl->next;
						continue;
					}
#endif
				}
				if(!roq && ssuite && ssuite->value && atoi(ssuite->value) != 32 && atoi(ssuite->value) != 80) {
				...
				if(media != NULL) {
					...
					while(ml) {
...
#ifdef HAVE_IMQUIC
						if(roq && data) {
							JANUS_LOG(LOG_WARN, "  -- Unsupported media type '%s' when using RoQ, skipping in '%s'...\n", type->value, cat->name);
							ml = ml->next;
							continue;
						}
#endif
...
#ifdef HAVE_IMQUIC
						janus_config_item *flowid = janus_config_get(config, m, janus_config_type_item, "roqflow");
#endif
						...
						if((audio || video) &&
								((!roq && (port == NULL || port->value == NULL || atoi(port->value) < 0)) ||
						...
						janus_streaming_rtp_source_stream *stream = janus_streaming_create_rtp_source_stream(
							...
							roq, (flowid && flowid->value ? g_ascii_strtoull(flowid->value, 0, 10) : 0),
					...
					if(doaudio) {
						...
						janus_streaming_rtp_source_stream *stream = janus_streaming_create_rtp_source_stream(
							cat->name, g_list_length(streams), FALSE, 0,
							"audio", "a", "audio", NULL,
...
#ifdef HAVE_IMQUIC
	janus_mutex_lock(&roq_mutex);
	g_hash_table_destroy(roq_servers);
	roq_servers = NULL;
	janus_mutex_unlock(&roq_mutex);
#endif
...
static json_t *janus_streaming_process_synchronous_request(janus_streaming_session *session, json_t *message) {
	...
	if(!strcasecmp(request_text, "list")) {
		...
		if(!strcasecmp(type_text, "rtp") || !strcasecmp(type_text, "roq")) {
			/* RTP live source (e.g., from gstreamer/ffmpeg/vlc/etc.), or RTP over QUIC */
			...
			/* Check if this is an RTP over QUIC mountpoint, or a "standard" one */
			gboolean roq = !strcasecmp(type_text, "roq");
			janus_streaming_roq_server *rs = NULL;
			if(roq) {
#ifndef HAVE_IMQUIC
				JANUS_LOG(LOG_ERR, "RoQ support unavailable\n");
				error_code = JANUS_STREAMING_ERROR_CANT_CREATE;
				g_snprintf(error_cause, 512, "RoQ support unavailable");
				janus_mutex_lock(&mountpoints_mutex);
				g_hash_table_remove(mountpoints_temp, string_ids ? (gpointer)mpid_str : (gpointer)&mpid);
				janus_mutex_unlock(&mountpoints_mutex);
				goto prepare_response;
#else
				const char *rsname = json_string_value(json_object_get(root, "roq"));
				janus_mutex_lock(&roq_mutex);
				rs = rsname ? g_hash_table_lookup(roq_servers, rsname) : NULL;
				janus_mutex_unlock(&roq_mutex);
				if(rs == NULL) {
					JANUS_LOG(LOG_ERR, "No such RoQ server\n");
					error_code = JANUS_STREAMING_ERROR_CANT_CREATE;
					g_snprintf(error_cause, 512, "No such RoQ server");
					janus_mutex_lock(&mountpoints_mutex);
					g_hash_table_remove(mountpoints_temp, string_ids ? (gpointer)mpid_str : (gpointer)&mpid);
					janus_mutex_unlock(&mountpoints_mutex);
					goto prepare_response;
			...
			if(!roq && ssuite && json_integer_value(ssuite) != 32 && json_integer_value(ssuite) != 80) {
			...
			if(media != NULL) {
				...
				for(i=0; i<json_array_size(media); i++) {
...
#ifdef HAVE_IMQUIC
						if(roq) {
							JANUS_LOG(LOG_WARN, "  -- Unsupported media type '%s' when using RoQ, skipping...\n", mtype);
							continue;
						}
#endif
...
#ifdef HAVE_IMQUIC
					if(roq) {
						json_t *roqflow = json_object_get(root, "roqflow");
						if(roqflow == NULL) {
							JANUS_LOG(LOG_ERR, "Missing element (roqflow)\n");
							error_code = JANUS_STREAMING_ERROR_MISSING_ELEMENT;
							g_snprintf(error_cause, 512, "Missing element (roqflow)");
							janus_mutex_lock(&mountpoints_mutex);
							g_hash_table_remove(mountpoints_temp, string_ids ? (gpointer)mpid_str : (gpointer)&mpid);
							janus_mutex_unlock(&mountpoints_mutex);
							goto prepare_response;
					...
					/* Create the source stream */
					janus_streaming_rtp_source_stream *stream = janus_streaming_create_rtp_source_stream(
						name ? (char *)json_string_value(name) : NULL, g_list_length(streams), roq, flow_id,
...
#ifdef HAVE_IMQUIC
				if(roq) {
					JANUS_LOG(LOG_ERR, "Legacy API not supported for RoQ mountpoints\n");
					error_code = JANUS_STREAMING_ERROR_CANT_CREATE;
					g_snprintf(error_cause, 512, "Legacy API not supported for RoQ mountpoints");
					janus_mutex_lock(&mountpoints_mutex);
					g_hash_table_remove(mountpoints_temp, string_ids ? (gpointer)mpid_str : (gpointer)&mpid);
					janus_mutex_unlock(&mountpoints_mutex);
					goto prepare_response;
				...
				if(doaudio) {
					...
					/* Create the audio source stream */
					janus_streaming_rtp_source_stream *stream = janus_streaming_create_rtp_source_stream(
						name ? (char *)json_string_value(name) : NULL, g_list_length(streams), FALSE, 0,
						"audio", "a", "audio", NULL,
...
#ifdef HAVE_IMQUIC
			if(mp->srs)
				janus_config_add(config, c, janus_config_item_create("roq", mp->srs->name));
#endif
			if(mp->metadata)
...
#ifdef HAVE_IMQUIC
					if(mp->srs) {
						g_snprintf(value, BUFSIZ, "%"SCNu64, stream->roq_flow_id);
						janus_config_add(config, m, janus_config_item_create("roqflow", value));
					}
#endif
					if(stream->port[0] > 0) {
		...
		goto prepare_response;
#ifdef HAVE_IMQUIC
	} else if(!strcasecmp(request_text, "roq_create")) {
		/* A request to create a new RoQ server, to potentially feed a mountpoint */
		JANUS_VALIDATE_JSON_OBJECT(root, roq_create_parameters,
			error_code, error_cause, TRUE,
			JANUS_STREAMING_ERROR_MISSING_ELEMENT, JANUS_STREAMING_ERROR_INVALID_ELEMENT);
		if(error_code != 0)
			goto prepare_response;
		...
		janus_mutex_lock(&roq_mutex);
		if(g_hash_table_lookup(roq_servers, name) != NULL) {
			janus_mutex_unlock(&roq_mutex);
			JANUS_LOG(LOG_ERR, "RoQ server '%s' already exists\n", name);
			error_code = JANUS_STREAMING_ERROR_CANT_CREATE;
			g_snprintf(error_cause, 512, "RoQ server '%s' already exists", name);
			goto prepare_response;
		}
		/* Create a new RoQ server */
		...
		if(rs == NULL) {
			janus_mutex_unlock(&roq_mutex);
			JANUS_LOG(LOG_ERR, "Error creating RoQ server '%s' instance\n", name);
			error_code = JANUS_STREAMING_ERROR_CANT_CREATE;
			g_snprintf(error_cause, 512, "Error creating RoQ server '%s' instance", name);
			goto prepare_response;
		}
		janus_streaming_roq_server *srs = g_malloc0(sizeof(janus_streaming_roq_server));
		srs->name = g_strdup(name);
		srs->server = rs;
		rs->metadata = srs;
		janus_mutex_init(&srs->mutex);
		janus_refcount_init(&srs->ref, janus_streaming_roq_server_free);
		g_hash_table_insert(roq_servers, g_strdup(name), srs);
		janus_mutex_unlock(&roq_mutex);
		/* Send a success response back */
		response = json_object();
		json_object_set_new(response, "streaming", json_string("roq_created"));
		json_object_set_new(response, "name", json_string(name));
		/* Also notify event handlers */
		if(notify_events && gateway->events_is_enabled()) {
			json_t *info = json_object();
			json_object_set_new(info, "event", json_string("roq_created"));
			json_object_set_new(info, "name", json_string(name));
			gateway->notify_event(&janus_streaming_plugin, session ? session->handle : NULL, info);
		}
		goto prepare_response;
	} else if(!strcasecmp(request_text, "roq_list")) {
		/* A request to list existing RoQ servers */
		if(admin_key != NULL) {
			/* An admin key was specified: make sure it was provided, and that it's valid */
			JANUS_VALIDATE_JSON_OBJECT(root, adminkey_parameters,
				error_code, error_cause, TRUE,
		...
		json_t *list = json_array();
		/* Return a list of all available RoQ servers */
		janus_mutex_lock(&roq_mutex);
		GHashTableIter iter;
		gpointer value;
		g_hash_table_iter_init(&iter, roq_servers);
		while(g_hash_table_iter_next(&iter, NULL, &value)) {
			janus_streaming_roq_server *srs = value;
			json_t *rs = json_object();
			json_object_set_new(rs, "name", json_string(srs->name));
			json_array_append_new(list, rs);
		}
		janus_mutex_unlock(&roq_mutex);
		/* Send info back */
		response = json_object();
		json_object_set_new(response, "streaming", json_string("roq_servers"));
		json_object_set_new(response, "list", list);
		goto prepare_response;
	} else if(!strcasecmp(request_text, "roq_destroy")) {
		/* A request to destroy an existing RoQ server */
		JANUS_VALIDATE_JSON_OBJECT(root, roq_destroy_parameters,
			error_code, error_cause, TRUE,
			JANUS_STREAMING_ERROR_MISSING_ELEMENT, JANUS_STREAMING_ERROR_INVALID_ELEMENT);
		if(error_code != 0)
			goto prepare_response;
		const char *name = json_string_value(json_object_get(root, "name"));
		janus_mutex_lock(&roq_mutex);
		janus_streaming_roq_server *srs = g_hash_table_lookup(roq_servers, name);
		if(srs == NULL) {
			janus_mutex_unlock(&roq_mutex);
			JANUS_LOG(LOG_VERB, "No such RoQ server '%s'\n", name);
			error_code = JANUS_STREAMING_ERROR_NO_SUCH_MOUNTPOINT;
			g_snprintf(error_cause, 512, "No such RoQ server '%s'", name);
			goto prepare_response;
		}
		/* TODO Get rid of associated mountpoints */
		g_hash_table_remove(roq_servers, name);
		/* Send a success response back */
		response = json_object();
		json_object_set_new(response, "streaming", json_string("roq_destroyed"));
		json_object_set_new(response, "name", json_string(name));
		/* Also notify event handlers */
		if(notify_events && gateway->events_is_enabled()) {
			json_t *info = json_object();
			json_object_set_new(info, "event", json_string("roq_destroyed"));
			json_object_set_new(info, "name", json_string(name));
			gateway->notify_event(&janus_streaming_plugin, session ? session->handle : NULL, info);
		}
		goto prepare_response;
...
#endif
	if(roq && mtype == JANUS_STREAMING_MEDIA_DATA) {
		JANUS_LOG(LOG_WARN, "[%s] Mountpoint wants to do datachannel relaying, but this is a RoQ mountpoint...\n", name);
		return NULL;
	}
...
janus_streaming_mountpoint *janus_streaming_create_rtp_source(
		...
		GList *media, int srtpsuite, char *srtpcrypto, int threads, int rtp_collision,
		gboolean e2ee, gboolean playoutdelay_ext, int abscapturetime_src_ext_id) {
	JANUS_LOG(LOG_WARN, "[janus_streaming_create_rtp_source] %s\n", name);
	char id_num[30];
	if(!string_ids) {
	...
	janus_mutex_unlock(&mountpoints_mutex);
#ifdef HAVE_IMQUIC
	JANUS_LOG(LOG_WARN, "%s roq=%p\n", live_rtp->id_str, roq_server);
	if(roq_server != NULL) {
		/* The source is RoQ, add the mountpoint to the list of instances
		 * interested in the media and don't spawn a thread */
		janus_mutex_lock(&roq_server->mutex);
		roq_server->mountpoints = g_list_prepend(roq_server->mountpoints, live_rtp);
		janus_mutex_unlock(&roq_server->mutex);
		JANUS_LOG(LOG_WARN, "  -- %p\n", roq_server->mountpoints);
		return live_rtp;
	}
#endif
		...
		for(i=0; i<num; i++) {
			if(fds[i].revents & (POLLERR | POLLHUP)) {
				...
				if(stream->type == JANUS_STREAMING_MEDIA_AUDIO && fds[i].fd == stream->fd[0]) {
					...
					janus_streaming_process_packet(mountpoint, name, stream, FALSE, buffer, bytes, 0);
					...
					janus_streaming_process_packet(mountpoint, name, stream, FALSE, buffer, bytes, index);
					continue;
					...
					janus_streaming_process_packet(mountpoint, name, stream, TRUE, buffer, bytes, 0);
				}
			}
		}
	}
...
static void janus_streaming_process_packet(janus_streaming_mountpoint *mountpoint, char *name,
		janus_streaming_rtp_source_stream *stream, gboolean rtcp, char *buffer, int bytes, int index) {
	if(mountpoint == NULL || g_atomic_int_get(&mountpoint->destroyed) ||
			mountpoint->source == NULL || stream == NULL || buffer == NULL || bytes < 1)
		return;
	janus_streaming_rtp_source *source = mountpoint->source;
	janus_streaming_rtp_relay_packet packet = { 0 };
	gint64 now = janus_get_monotonic_time();
	if(stream->type == JANUS_STREAMING_MEDIA_AUDIO && !rtcp) {
		/* Got something audio (RTP) */
		...
		return;
	} else if(stream->type == JANUS_STREAMING_MEDIA_VIDEO && !rtcp) {
		/* Got something video (RTP) */
		if(!janus_is_rtp(buffer, bytes)) {
			/* Not an RTP packet? */
			return;
