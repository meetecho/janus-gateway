...
shared gint janus_dtls_srtp_init(const char* server_pem, const char* server_key);
/*! \brief Method to cleanup DTLS stuff before exiting */
shared void janus_dtls_srtp_cleanup(void);
/*! \brief Method to return a string representation (SHA-256) of the certificate fingerprint */
shared gchar *janus_dtls_get_local_fingerprint(void);
...
shared void janus_dtls_callback(const SSL *ssl, int where, int ret);
...
shared int janus_dtls_verify_callback(int preverify_ok, X509_STORE_CTX *ctx);
...
shared void janus_dtls_fd_bridge(janus_dtls_srtp *dtls);
...
shared void janus_dtls_wrap_sctp_data(janus_dtls_srtp *dtls, char *buf, int len);
...
shared int janus_dtls_send_sctp_data(janus_dtls_srtp *dtls, char *buf, int len);

/*! \brief Callback to be notified about incoming SCTP data (DataChannel) to forward to the handle
 * @param[in] dtls The janus_dtls_srtp instance to use
 * @param[in] buf The data buffer
 * @param[in] len The data length */
shared void janus_dtls_notify_data(janus_dtls_srtp *dtls, char *buf, int len);
#endif
