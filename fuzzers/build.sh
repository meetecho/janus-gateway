...
if [ "$ENV" == "oss-fuzz" ]; then
	...
	if [ ! -z $FUZZ_ENGINE ]; then
		FUZZ_LDFLAGS=${LDFLAGS-"-O1 -fno-omit-frame-pointer -g -ggdb3 -fsanitize=address,undefined -fsanitize-address-use-after-scope -fsanitize=fuzzer-no-link"}
	else
