...
export class SessionManager {
  ...
  /** Helper function to check if the session is one we are managing. */
  private sessionManaged(session: Session): ManagedSession | undefined {
  ...
  private setupLocalMedia(session: Session): void {
    ...
    const mediaLocal =
      typeof this.options.media.local === "function" ? this.options.media.local(session) : this.options.media.local;
  ...
  private setupRemoteMedia(session: Session): void {
    ...
    // and save the info with the managed session so we can clean it up later.
    const mediaRemote =
      typeof this.options.media.remote === "function" ? this.options.media.remote(session) : this.options.media.remote;
