...
shared janus_config *janus_config_parse(const char *config_file);
/*! \brief Method to create a new, empty, configuration
 * @param[in] name Name to give to the configuration
 * @returns A pointer to a valid janus_config instance if successful, NULL otherwise */ 
shared janus_config *janus_config_create(const char *name);
...
shared GList *janus_config_get_categories(janus_config *config);
...
shared janus_config_category *janus_config_get_category(janus_config *config, const char *name);
...
shared GList *janus_config_get_items(janus_config_category *category);
...
shared janus_config_item *janus_config_get_item(janus_config_category *category, const char *name);
 ...
 * @param[in] name The name of the item
 * @returns A pointer to the janus_config_item instance if successful, NULL otherwise */ 
shared janus_config_item *janus_config_get_item_drilldown(janus_config *config, const char *category, const char *name);
 ...
 * @returns A pointer to the janus_config_category instance if successful, NULL otherwise */ 
shared janus_config_category *janus_config_add_category(janus_config *config, const char *category);
...
shared int janus_config_remove_category(janus_config *config, const char *category);
 ...
 * @returns A pointer to the janus_config_item instance if successful, NULL otherwise */ 
shared janus_config_item *janus_config_add_item(janus_config *config, const char *category, const char *name, const char *value);
 ...
 * @param[in] name The name of the item
 * @returns 0 if successful, a negative integer otherwise */ 
shared int janus_config_remove_item(janus_config *config, const char *category, const char *name);
/*! \brief Helper method to print a configuration on the standard output
 * @param[in] config The configuration to print */
shared void janus_config_print(janus_config *config);
 ...
 * @returns 0 if successful, a negative integer otherwise */
shared int janus_config_save(janus_config *config, const char *folder, const char *filename);
/*! \brief Destroy a configuration container instance
 * @param[in] config The configuration to destroy */
shared void janus_config_destroy(janus_config *config);
