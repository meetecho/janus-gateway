...
void janus_plugin_relay_data(janus_plugin_session *plugin_session, char *label, char *protocol, gboolean textdata, char *buf, int len);
...
void janus_plugin_relay_data(janus_plugin_session *plugin_session, char *label, char *protocol, gboolean textdata, char *buf, int len) {
	...
	if(!handle || janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_STOP)
		...
		return;
#ifdef HAVE_SCTP
	janus_ice_relay_data(handle, label, protocol, textdata, buf, len);
#else
