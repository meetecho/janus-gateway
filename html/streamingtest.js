						...
						janus.attach(
							{
								...
								onremotetrack: function(track, mid, on) {
									...
									if($('#'+mstreamId+' audio').length === 0 && $('#'+mstreamId+' video').length === 0) {
										addButtons = true;
									}
									if(track.kind === "audio") {
										...
										$('#'+mstreamId).append('<audio class="hide" id="remotevideo' + mid + '" autoplay playsinline/>');
										if(remoteVideos === 0) {
											// No video, at least for now: show a placeholder
											if($('#'+mstreamId+' .no-video-container').length === 0) {
												$('#'+mstreamId).append(
										...
										$('#'+mstreamId).append('<video class="rounded centered hide" id="remotevideo' + mid + '" width=320 height=240 autoplay playsinline/>');
									...
									$("#remotevideo" + mid).bind("playing", function (ev) {
										$('.waitingvideo').remove();
										if(spinner[mid] !== null && spinner[mid] !== undefined)
											spinner[mid].stop();
										spinner[mid] = null;
										if(!this.videoWidth)
											return;
...
function startStream() {
	...
	if(selectedStream === null || selectedStream === undefined || !streamsList[selectedStream]) {
	...
	if(streamsList[selectedStream].legacy) {
		...
		if($('#mstream0').length === 0) {
			addPanel("0", mid);
			// No remote video yet
			$('#mstream0').append('<video class="rounded centered waitingvideo" id="waitingvideo0" width=320 height=240 />');
		}
		if(mid) {
			if(spinner[mid] == null) {
		...
		for(mi in streamsList[selectedStream].media) {
			...
			if($('#mstream'+mid).length === 0) {
				...
				// No remote media yet
				$('#mstream'+mid).append('<video class="rounded centered waitingvideo" id="waitingvideo'+mid+'" width=320 height=240 />');
			}
			if(spinner[mid] == null) {
...
function addSimulcastButtons(mid, temporal) {
	...
	$('#m-'+mid+'-sl-0').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			var mid = $(this).attr('id').split('m-')[1].split('-sl')[0];
	...
	$('#m-'+mid+'-tl-0').removeClass('btn-primary btn-success').addClass('btn-primary')
		.unbind('click').click(function() {
			var mid = $(this).attr('id').split('m-')[1].split('-tl')[0];
