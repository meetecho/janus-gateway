	...
	Janus.init({debug: true, callback: function() {
		...
		$('#start').click(function() {
			...
			if(!Janus.isWebrtcSupported()) {
				bootbox.alert("不支持 WebRTC... ");//No WebRTC support...
				return;
			}
			// Create session
			janus = new Janus(
						...
						janus.attach(
							{
								...
								success: function(pluginHandle) {
									...
									$('#registernowroom').removeClass('hide').show();
									$('#registernowpwd').removeClass('hide').show();
									$('#registernow').removeClass('hide').show();
									$('#register').click(registerRoom);
									$('#room').focus();
									$('#start').removeAttr('disabled').html("停止")//stop
										.click(function() {
								...
								onmessage: function(msg, jsep) {
									...
									if(event != undefined && event != null) {
										if(event === "joined") {
											...
											$('#sessionid').html(janus.getSessionId());
								...
								onlocalstream: function(stream) {
									...
									if($('#myvideo').length === 0) {
										...
										// Add a 'mute' button
										$('#videolocal').append('<button class="btn btn-warning btn-xs" id="mute" style="position: absolute; bottom: 0px; left: 0px; margin: 15px;">静音</button>');//Mute
										$('#mute').click(toggleMute);
										// Add an 'unpublish' button
										$('#videolocal').append('<button class="btn btn-warning btn-xs" id="unpublish" style="position: absolute; bottom: 0px; right: 0px; margin: 15px;">取消发布</button>');//Unpublish
									...
									if(videoTracks === null || videoTracks === undefined || videoTracks.length === 0) {
										...
										$('#videolocal').append(
											'<div class="no-video-container">' +
												...
												'<span class="no-video-text" style="font-size: 16px;">未找到网络摄像头</span>' + //No webcam available
											'</div>');
									}
								},
								...
								oncleanup: function() {
									...
									mystream = null;
									muted = false;
									$('#videolocal').html('<button id="publish" class="btn btn-primary">发布</button>');//Publish
...
function registerRoom() {
	if($('#room').length === 0 || $('#username').length === 0) {
		...
		if(room === "") {
			...
			$('#roompwd').removeAttr('disabled');
			$('#username').removeAttr('disabled');
			$('#register').removeAttr('disabled').click(registerRoom);
			return;
		}
		if(/[^0-9]/.test(room)) {
			$('#you')
				.removeClass().addClass('label label-warning')
				.html('输入的房间号必须是数字');
			$('#room').removeAttr('disabled').val("");
			$('#roompwd').removeAttr('disabled').val("");
		...
		if(/[^a-zA-Z0-9]/.test(roompwd)) {
			$('#you')
				.removeClass().addClass('label label-warning')
				.html('输入的房间密码必须是字母或数字');
			$('#room').removeAttr('disabled').val("");
			$('#roompwd').removeAttr('disabled').val("");
			$('#username').removeAttr('disabled').val("");
			$('#register').removeAttr('disabled').click(registerRoom);
		...
		room = parseInt(room);
		var register = { "request": "join", "room": room, "ptype": "publisher", "secret": roompwd, "display": username };
		myroom = room;
		myroompwd = roompwd;
		myusername = username;
...
function toggleMute() {
	...
	muted = !muted;
	$('#mute').html(muted ? "取消静音" : "静音");//"Unmute" : "Mute"
}
	...
	janus.attach(
		{
			...
			success: function(pluginHandle) {
				...
				var listen = { "request": "join", "room": myroom, "ptype": "listener", "secret": myroompwd, "feed": id };
			...
			onremotestream: function(stream) {
				...
				if(videoTracks === null || videoTracks === undefined || videoTracks.length === 0 || videoTracks[0].muted) {
					...
					$('#videoremote'+remoteFeed.rfindex).append(
						'<div class="no-video-container">' +
							'<i class="fa fa-video-camera fa-5 no-video-icon" style="height: 100%;"></i>' +
							'<span class="no-video-text" style="font-size: 16px;">没有远程视频</span>' +//No remote video available
						'</div>');
				}
