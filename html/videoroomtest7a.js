...
{
  var base64Encoded = CryptoJS.enc.Base64.stringify(data);
  var urlSafe = base64Encoded.replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, ''); //transform to URL Safe encoding
    return urlSafe;
}

var environment = "dev"

function getToken(streamid )
{
    var d = new Date();
    var t = d.getTime();
    var dateBase64 = btoa(t);
    var r = Math.floor(Math.random(256));
    var nonceBase64 = btoa(r);
    var message = streamid + ":" + dateBase64 + ":" + nonceBase64;
    var key = {dev:"SECRET_CREATE_JOIN_KEY_DEV",
               qa:"SECRET_CREATE_JOIN_KEY_QA",
               stage:"SECRET_CREATE_JOIN_KEY_STAGE",
               prod:"SECRET_CREATE_JOIN_KEY_PROD"};

    var hash = CryptoJS.HmacSHA256(message, key[environment]);
    hashInBase64 = base64URLEncode(hash);

    var token = dateBase64 + ":" + nonceBase64 + ":" + hashInBase64;
								...
								onmessage: function(msg, jsep) {
									...
									if(event != undefined && event != null) {
										if(event === "joined") {
											...
											//PVL-NO-AUDIO publishOwnFeed(true);
											...
											if(msg["publishers"] !== undefined && msg["publishers"] !== null) {
												...
												for(var f in list) {
													...
													newRemoteFeed(id, display, audio, video);
                                                                                        ...
                                                                                        //var rtpforward ={ "request" : "rtp_forward",
                                                                                        //                "room" : msg["room"],
                                                                                        //                "publisher_id" : myid,
                                                                                        //                "host" : "127.0.0.1",
                                                                                        //                "audio_port" : 60000,
...
                                                                                       //                 "video_pt" : 102,
								...
								oncleanup: function() {
									...
									$('#publish').click(function() { publishOwnFeed(false); }); //PVL-NO-AUDIO
									$("#videolocal").parent().parent().unblock();
...
function registerUsername() {
	if($('#username').length === 0) {
		...
		if(/[^a-zA-Z0-9]/.test(username)) {
           ...
           // Create a new room
            var roomnumber = streamid;

           var roomid= getRandomInt(10000,20000);
           var tokenstr = getToken(roomid);
           var create = { "request": "create","room":roomid.toString(), "wgwtoken":tokenstr,"fir_freq":3,"videocodec":"vp8","description":"Create New Room ", "bitrate": 500000, "publishers": 1 };
           sfutest.send({"message": create, success: function(result) {
                var event = result["videoroom"];
                Janus.debug("Event: " + event);
                if(event != undefined && event != null) {
                   if(event === "created") {
                        // Our own video room  session has been created, join it
                        myroom = result["room"];
                        Janus.log("Video Room created: " + myroom);
                        $("#sdpDataTag").html(myroom);
                        var register = { "request": "join", "room": myroom, "wgwtoken":tokenstr, "ptype": "publisher", "display": username };
                        myusername = username;
