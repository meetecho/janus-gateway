...
/* Plugin information */
#define JANUS_AUVIOUSSIP_VERSION			7
#define JANUS_AUVIOUSSIP_VERSION_STRING	"0.0.7"
#define JANUS_AUVIOUSSIP_DESCRIPTION		"This is a auvious SIP plugin for Janus, allowing WebRTC peers to register at a SIP server and call SIP user agents through a Janus instance."
#define JANUS_AUVIOUSSIP_NAME				"JANUS SIP plugin"
#define JANUS_AUVIOUSSIP_AUTHOR			"Meetecho s.r.l."
#define JANUS_AUVIOUSSIP_PACKAGE			"janus.plugin.auvioussip"

/* Plugin methods */
janus_plugin *create(void);
int janus_auvioussip_init(janus_callbacks *callback, const char *config_path);
void janus_auvioussip_destroy(void);
int janus_auvioussip_get_api_compatibility(void);
int janus_auvioussip_get_version(void);
const char *janus_auvioussip_get_version_string(void);
const char *janus_auvioussip_get_description(void);
const char *janus_auvioussip_get_name(void);
const char *janus_auvioussip_get_author(void);
const char *janus_auvioussip_get_package(void);
void janus_auvioussip_create_session(janus_plugin_session *handle, int *error);
struct janus_plugin_result *janus_auvioussip_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep);
void janus_auvioussip_setup_media(janus_plugin_session *handle);
void janus_auvioussip_incoming_rtp(janus_plugin_session *handle, janus_plugin_rtp *packet);
void janus_auvioussip_incoming_rtcp(janus_plugin_session *handle, janus_plugin_rtcp *packet);
void janus_auvioussip_hangup_media(janus_plugin_session *handle);
void janus_auvioussip_destroy_session(janus_plugin_session *handle, int *error);
json_t *janus_auvioussip_query_session(janus_plugin_session *handle);

/* Plugin setup */
static janus_plugin janus_auvioussip_plugin =
	JANUS_PLUGIN_INIT (
		.init = janus_auvioussip_init,
		.destroy = janus_auvioussip_destroy,

		.get_api_compatibility = janus_auvioussip_get_api_compatibility,
		.get_version = janus_auvioussip_get_version,
		.get_version_string = janus_auvioussip_get_version_string,
		.get_description = janus_auvioussip_get_description,
		.get_name = janus_auvioussip_get_name,
		.get_author = janus_auvioussip_get_author,
		.get_package = janus_auvioussip_get_package,

		.create_session = janus_auvioussip_create_session,
		.handle_message = janus_auvioussip_handle_message,
		.setup_media = janus_auvioussip_setup_media,
		.incoming_rtp = janus_auvioussip_incoming_rtp,
		.incoming_rtcp = janus_auvioussip_incoming_rtcp,
		.hangup_media = janus_auvioussip_hangup_media,
		.destroy_session = janus_auvioussip_destroy_session,
		.query_session = janus_auvioussip_query_session,
	);

/* Plugin creator */
janus_plugin *create(void) {
	JANUS_LOG(LOG_VERB, "%s created!\n", JANUS_AUVIOUSSIP_NAME);
	return &janus_auvioussip_plugin;
}
...
static void *janus_auvioussip_handler(void *data);
static void janus_auvioussip_hangup_media_internal(janus_plugin_session *handle);

typedef struct janus_auvioussip_message {
	janus_plugin_session *handle;
	char *transaction;
	json_t *message;
	json_t *jsep;
} janus_auvioussip_message;
static GAsyncQueue *messages = NULL;
static janus_auvioussip_message exit_message;


typedef enum {
	janus_auvioussip_registration_status_disabled = -2,
	janus_auvioussip_registration_status_failed = -1,
	janus_auvioussip_registration_status_unregistered = 0,
	janus_auvioussip_registration_status_registering,
	janus_auvioussip_registration_status_registered,
	janus_auvioussip_registration_status_unregistering,
} janus_auvioussip_registration_status;

static const char *janus_auvioussip_registration_status_string(janus_auvioussip_registration_status status) {
	switch(status) {
		case janus_auvioussip_registration_status_disabled:
			return "disabled";
		case janus_auvioussip_registration_status_failed:
			return "failed";
		case janus_auvioussip_registration_status_unregistered:
			return "unregistered";
		case janus_auvioussip_registration_status_registering:
			return "registering";
		case janus_auvioussip_registration_status_registered:
			return "registered";
		case janus_auvioussip_registration_status_unregistering:
...
typedef enum {
	janus_auvioussip_call_status_idle = 0,
	janus_auvioussip_call_status_inviting,
	janus_auvioussip_call_status_invited,
	janus_auvioussip_call_status_incall,
	janus_auvioussip_call_status_closing,
} janus_auvioussip_call_status;

static const char *janus_auvioussip_call_status_string(janus_auvioussip_call_status status) {
	switch(status) {
		case janus_auvioussip_call_status_idle:
			return "idle";
		case janus_auvioussip_call_status_inviting:
			return "inviting";
		case janus_auvioussip_call_status_invited:
			return "invited";
		case janus_auvioussip_call_status_incall:
			return "incall";
		case janus_auvioussip_call_status_closing:
			return "closing";
		default:
			return "unknown";
	}
...
typedef enum {
	janus_auvioussip_secret_type_plaintext = 1,
	janus_auvioussip_secret_type_hashed = 2,
	janus_auvioussip_secret_type_unknown
} janus_auvioussip_secret_type;

typedef struct janus_auvioussip_account {
	char *identity;
	...
	char *secret;
	janus_auvioussip_secret_type secret_type;
	int sip_port;
	char *proxy;
	char *outbound_proxy;
	janus_auvioussip_registration_status registration_status;
} janus_auvioussip_account;

typedef struct janus_auvioussip_media {
	char *remote_ip;
	gboolean earlymedia;
	gboolean update;
	...
	int pipefd[2];
	gboolean updated;
} janus_auvioussip_media;

typedef struct janus_auvioussip_session {
	janus_plugin_session *handle;
	ssip_t *stack;
	janus_auvioussip_account account;
	janus_auvioussip_call_status status;
	janus_auvioussip_media media;
	char *transaction;
	char *callee;
	char *callid;
...
} janus_auvioussip_session;
static GHashTable *sessions;
static GHashTable *identities;
static GHashTable *callids;
static janus_mutex sessions_mutex = JANUS_MUTEX_INITIALIZER;

static void janus_auvioussip_srtp_cleanup(janus_auvioussip_session *session);

static void janus_auvioussip_session_destroy(janus_auvioussip_session *session) {
	if(session && g_atomic_int_compare_and_exchange(&session->destroyed, 0, 1))
		janus_refcount_decrease(&session->ref);
}

static void janus_auvioussip_session_free(const janus_refcount *session_ref) {
	janus_auvioussip_session *session = janus_refcount_containerof(session_ref, janus_auvioussip_session, ref);
	...
	janus_auvioussip_srtp_cleanup(session);
	g_free(session);
}

static void janus_auvioussip_message_free(janus_auvioussip_message *msg) {
	if(!msg || msg == &exit_message)
		return;

	if(msg->handle && msg->handle->plugin_handle) {
		janus_auvioussip_session *session = (janus_auvioussip_session *)msg->handle->plugin_handle;
		janus_refcount_decrease(&session->ref);
	}
	msg->handle = NULL;
...
struct ssip_s {
	...
	janus_auvioussip_session *session;
};


/* SRTP stuff (in case we need SDES) */
static int janus_auvioussip_srtp_set_local(janus_auvioussip_session *session, gboolean video, char **profile, char **crypto) {
	if(session == NULL)
		return -1;
...
static int janus_auvioussip_srtp_set_remote(janus_auvioussip_session *session, gboolean video, const char *profile, const char *crypto) {
...
/* Sofia Event thread */
gpointer janus_auvioussip_sofia_thread(gpointer user_data);
/* Sofia callbacks */
void janus_auvioussip_sofia_callback(nua_event_t event, int status, char const *phrase, nua_t *nua, nua_magic_t *magic, nua_handle_t *nh, nua_hmagic_t *hmagic, sip_t const *sip, tagi_t tags[]);
/* SDP parsing and manipulation */
void janus_auvioussip_sdp_process(janus_auvioussip_session *session, janus_sdp *sdp, gboolean answer, gboolean update, gboolean *changed);
char *janus_auvioussip_sdp_manipulate(janus_auvioussip_session *session, janus_sdp *sdp, gboolean answer);
/* Media */
static int janus_auvioussip_allocate_local_ports(janus_auvioussip_session *session);
static void *janus_auvioussip_relay_thread(void *data);
static void janus_auvioussip_media_cleanup(janus_auvioussip_session *session);


/* URI parsing utilies */

#define JANUS_AUVIOUSSIP_URI_MAXLEN	1024
typedef struct {
	char data[JANUS_AUVIOUSSIP_URI_MAXLEN];
	url_t url[1];
} janus_auvioussip_uri_t;

/* Parses a SIP URI (SIPS is not supported), returns 0 on success, -1 otherwise */
static int janus_auvioussip_parse_uri(janus_auvioussip_uri_t *sip_uri, const char *data) {
	g_strlcpy(sip_uri->data, data, JANUS_AUVIOUSSIP_URI_MAXLEN);
...
static int janus_auvioussip_parse_proxy_uri(janus_auvioussip_uri_t *sip_uri, const char *data) {
	g_strlcpy(sip_uri->data, data, JANUS_AUVIOUSSIP_URI_MAXLEN);
...
/* Error codes */
#define JANUS_AUVIOUSSIP_ERROR_UNKNOWN_ERROR		499
#define JANUS_AUVIOUSSIP_ERROR_NO_MESSAGE			440
#define JANUS_AUVIOUSSIP_ERROR_INVALID_JSON		441
#define JANUS_AUVIOUSSIP_ERROR_INVALID_REQUEST		442
#define JANUS_AUVIOUSSIP_ERROR_MISSING_ELEMENT		443
#define JANUS_AUVIOUSSIP_ERROR_INVALID_ELEMENT		444
#define JANUS_AUVIOUSSIP_ERROR_ALREADY_REGISTERED	445
#define JANUS_AUVIOUSSIP_ERROR_INVALID_ADDRESS		446
#define JANUS_AUVIOUSSIP_ERROR_WRONG_STATE			447
#define JANUS_AUVIOUSSIP_ERROR_MISSING_SDP			448
#define JANUS_AUVIOUSSIP_ERROR_LIBSOFIA_ERROR		449
#define JANUS_AUVIOUSSIP_ERROR_IO_ERROR			450
#define JANUS_AUVIOUSSIP_ERROR_RECORDING_ERROR		451
#define JANUS_AUVIOUSSIP_ERROR_TOO_STRICT			452


/* Random string helper (for call-ids) */
static char charset[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
static void janus_auvioussip_random_string(int length, char *buffer) {
	if(length > 0 && buffer) {
...
static void janus_auvioussip_parse_custom_headers(json_t *root, char *custom_headers) {
	custom_headers[0] = '\0';
 ...
 * messages are more easily recoverable in janus_auvioussip_sofia_callback) */
char sofia_log[2048];
char call_id[255];
gboolean skip = FALSE, started = FALSE, append = FALSE;
static void janus_auvioussip_sofia_logger(void *stream, char const *fmt, va_list ap) {
	if(!fmt)
		return;
	char line[255];
	...
	if(append) {
		...
		if(line[3] == '-') {
			if(!started) {
				...
				janus_auvioussip_session *session = NULL;
				...
				if(session) {
					...
					gateway->notify_event(&janus_auvioussip_plugin, session->handle, info);
					janus_refcount_decrease(&session->ref);
				} else {
...
int janus_auvioussip_init(janus_callbacks *callback, const char *config_path) {
	...
	/* Read configuration */
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.jcfg", config_path, JANUS_AUVIOUSSIP_PACKAGE);
	JANUS_LOG(LOG_VERB, "Configuration file: %s\n", filename);
	janus_config *config = janus_config_parse(filename);
	if(config == NULL) {
		JANUS_LOG(LOG_WARN, "Couldn't find .jcfg configuration file (%s), trying .cfg\n", JANUS_AUVIOUSSIP_PACKAGE);
		g_snprintf(filename, 255, "%s/%s.cfg", config_path, JANUS_AUVIOUSSIP_PACKAGE);
		JANUS_LOG(LOG_VERB, "Configuration file: %s\n", filename);
	...
	if(config != NULL) {
		...
		if(item && item->value)
			...
			user_agent = g_strdup("Janus WebRTC Server SIP Plugin "JANUS_AUVIOUSSIP_VERSION_STRING);
	...
	if(notify_events && callback->events_is_enabled()) {
		...
		su_log_redirect(NULL, janus_auvioussip_sofia_logger, NULL);
	}

	sessions = g_hash_table_new_full(NULL, NULL, NULL, (GDestroyNotify)janus_auvioussip_session_destroy);
	identities = g_hash_table_new(g_str_hash, g_str_equal);
	callids = g_hash_table_new(g_str_hash, g_str_equal);
	messages = g_async_queue_new_full((GDestroyNotify) janus_auvioussip_message_free);
	...
	GError *error = NULL;
	handler_thread = g_thread_try_new("sip handler", janus_auvioussip_handler, NULL, &error);
	if(error != NULL) {
...
int janus_auvioussip_get_version(void) {
	return JANUS_AUVIOUSSIP_VERSION;
}

const char *janus_auvioussip_get_version_string(void) {
	return JANUS_AUVIOUSSIP_VERSION_STRING;
}

const char *janus_auvioussip_get_description(void) {
	return JANUS_AUVIOUSSIP_DESCRIPTION;
}

const char *janus_auvioussip_get_name(void) {
	return JANUS_AUVIOUSSIP_NAME;
}

const char *janus_auvioussip_get_author(void) {
	return JANUS_AUVIOUSSIP_AUTHOR;
}

const char *janus_auvioussip_get_package(void) {
	return JANUS_AUVIOUSSIP_PACKAGE;
}

static janus_auvioussip_session *janus_auvioussip_lookup_session(janus_plugin_session *handle) {
	janus_auvioussip_session *session = NULL;
	if (g_hash_table_contains(sessions, handle)) {
		session = (janus_auvioussip_session *)handle->plugin_handle;
	}
	return session;
}
...
void janus_auvioussip_create_session(janus_plugin_session *handle, int *error) {
	...
	janus_auvioussip_session *session = g_malloc0(sizeof(janus_auvioussip_session));
	...
	session->account.authuser = NULL;
	session->account.secret = NULL;
	session->account.secret_type = janus_auvioussip_secret_type_unknown;
	session->account.sip_port = 0;
	session->account.proxy = NULL;
	session->account.outbound_proxy = NULL;
	session->account.registration_status = janus_auvioussip_registration_status_unregistered;
	session->status = janus_auvioussip_call_status_idle;
	session->stack = NULL;
	session->transaction = NULL;
	session->callee = NULL;
	session->callid = NULL;
	...
	janus_refcount_init(&session->ref, janus_auvioussip_session_free);
...
void janus_auvioussip_destroy_session(janus_plugin_session *handle, int *error) {
	...
	janus_mutex_lock(&sessions_mutex);
	janus_auvioussip_session *session = janus_auvioussip_lookup_session(handle);
	if(!session) {
		janus_mutex_unlock(&sessions_mutex);
	...
	janus_auvioussip_hangup_media_internal(handle);
	/* Shutdown the NUA */
...
json_t *janus_auvioussip_query_session(janus_plugin_session *handle) {
	...
	json_object_set_new(info, "registration_status", json_string(janus_auvioussip_registration_status_string(session->account.registration_status)));
	json_object_set_new(info, "call_status", json_string(janus_auvioussip_call_status_string(session->status)));
	if(session->callee) {
...
struct janus_plugin_result *janus_auvioussip_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep) {
	...
	janus_auvioussip_message *msg = g_malloc(sizeof(janus_auvioussip_message));
	msg->handle = handle;
...
void janus_auvioussip_setup_media(janus_plugin_session *handle) {
	JANUS_LOG(LOG_INFO, "[%s-%p] WebRTC media is now available\n", JANUS_AUVIOUSSIP_PACKAGE, handle);
	if(g_atomic_int_get(&stopping) || !g_atomic_int_get(&initialized))
		return;
	janus_mutex_lock(&sessions_mutex);
	janus_auvioussip_session *session = janus_auvioussip_lookup_session(handle);
...
void janus_auvioussip_incoming_rtp(janus_plugin_session *handle, janus_plugin_rtp *packet) {
	...
	if(gateway) {
		...
		janus_auvioussip_session *session = (janus_auvioussip_session *)handle->plugin_handle;
		if(!session || g_atomic_int_get(&session->destroyed)) {
			JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
			return;
		}
		if(session->status != janus_auvioussip_call_status_incall)
			return;
    ...
    uint16_t len = packet->length;
		...
		if(video) {
			...
			if(session->media.has_video && session->media.video_rtp_fd != -1) {
				...
				janus_recorder_save_frame(session->vrc, buf, packet->length);
    ...
    uint16_t len = packet->length;
		...
		if(video) {
			if(session->media.has_video && session->media.video_rtcp_fd != -1) {
				...
				janus_rtcp_fix_ssrc(NULL, buf, len, 1, session->media.video_ssrc, session->media.video_ssrc_peer);
...
static void janus_auvioussip_recorder_close(janus_auvioussip_session *session,
...
static void janus_auvioussip_hangup_media_internal(janus_plugin_session *handle) {
	...
	if(!session->media.ready && !session->relayer_thread) {
		janus_auvioussip_media_cleanup(session);
	}
	/* Get rid of the recorders, if available */
	janus_mutex_lock(&session->rec_mutex);
	janus_auvioussip_recorder_close(session, TRUE, TRUE, TRUE, TRUE);
	janus_mutex_unlock(&session->rec_mutex);
	if(!(session->status == janus_auvioussip_call_status_inviting ||
		 session->status == janus_auvioussip_call_status_invited ||
		 session->status == janus_auvioussip_call_status_incall)) {
		g_atomic_int_set(&session->establishing, 0);
		g_atomic_int_set(&session->established, 0);
		g_atomic_int_set(&session->hangingup, 0);
		return;
	...
	if(session->stack->s_nh_i != NULL && session->callee != NULL) {
		...
		session->media.ready = FALSE;
		session->media.on_hold = FALSE;
		session->status = janus_auvioussip_call_status_closing;
		...
		int ret = gateway->push_event(session->handle, &janus_auvioussip_plugin, NULL, event, NULL);
...
static void *janus_auvioussip_handler(void *data) {
	...
	janus_auvioussip_message *msg = NULL;
	int error_code = 0;
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(msg == &exit_message)
			break;
		if(msg->handle == NULL) {
			janus_auvioussip_message_free(msg);
			continue;
		}
		janus_mutex_lock(&sessions_mutex);
		janus_auvioussip_session *session = janus_auvioussip_lookup_session(msg->handle);
		if(!session) {
			janus_mutex_unlock(&sessions_mutex);
			JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
			janus_auvioussip_message_free(msg);
		...
		if(msg->message == NULL) {
			...
			error_code = JANUS_AUVIOUSSIP_ERROR_NO_MESSAGE;
			g_snprintf(error_cause, 512, "%s", "No message??");
			goto error;
		}
		if(!json_is_object(root)) {
			JANUS_LOG(LOG_ERR, "JSON error: not an object\n");
			error_code = JANUS_AUVIOUSSIP_ERROR_INVALID_JSON;
			g_snprintf(error_cause, 512, "JSON error: not an object");
			goto error;
		}
		JANUS_VALIDATE_JSON_OBJECT(root, request_parameters,
			error_code, error_cause, TRUE,
			JANUS_AUVIOUSSIP_ERROR_MISSING_ELEMENT, JANUS_AUVIOUSSIP_ERROR_INVALID_ELEMENT);
		if(error_code != 0)
			goto error;
		...
		if(!strcasecmp(request_text, "register")) {
			...
			if(session->account.registration_status > janus_auvioussip_registration_status_unregistered && !refresh) {
				JANUS_LOG(LOG_ERR, "Already registered (%s)\n", session->account.username);
				error_code = JANUS_AUVIOUSSIP_ERROR_ALREADY_REGISTERED;
			...
			if(do_register != NULL) {
				if(guest) {
					...
					error_code = JANUS_AUVIOUSSIP_ERROR_INVALID_ELEMENT;
			...
			if(proxy && !json_is_null(proxy)) {
				...
				janus_auvioussip_uri_t proxy_uri;
				if(janus_auvioussip_parse_proxy_uri(&proxy_uri, proxy_text) < 0) {
					JANUS_LOG(LOG_ERR, "Invalid proxy address %s\n", proxy_text);
					error_code = JANUS_AUVIOUSSIP_ERROR_INVALID_ADDRESS;
			...
			if (outbound_proxy && !json_is_null(outbound_proxy)) {
				...
				janus_auvioussip_uri_t outbound_proxy_uri;
				if (janus_auvioussip_parse_proxy_uri(&outbound_proxy_uri, obproxy_text) < 0) {
			...
			if(!username) {
				...
				error_code = JANUS_AUVIOUSSIP_ERROR_MISSING_ELEMENT;
			...
			janus_auvioussip_secret_type secret_type = janus_auvioussip_secret_type_plaintext;
			janus_auvioussip_uri_t username_uri;
			char user_id[256];
			/* Parse address */
			username_text = json_string_value(username);
			if(janus_auvioussip_parse_uri(&username_uri, username_text) < 0) {
			...
			if(guest) {
				...
				if(secret) {
					...
					secret_type = janus_auvioussip_secret_type_plaintext;
				} else {
					secret_text = json_string_value(ha1_secret);
					secret_type = janus_auvioussip_secret_type_hashed;
				}
				if(authuser) {
			...
			session->account.registration_status = janus_auvioussip_registration_status_registering;
			if(!refresh && session->stack == NULL) {
				...
				janus_refcount_increase(&session->ref);
				g_thread_try_new(tname, janus_auvioussip_sofia_thread, session, &error);
				if(error != NULL) {
					janus_refcount_decrease(&session->ref);
					JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the SIP Sofia thread...\n", error->code, error->message ? error->message : "??");
					error_code = JANUS_AUVIOUSSIP_ERROR_UNKNOWN_ERROR;
			...
			if(send_register) {
				...
				session->account.registration_status = janus_auvioussip_registration_status_disabled;
				result = json_object();
			...
			if(session->stack == NULL) {
				...
				error_code = JANUS_AUVIOUSSIP_ERROR_WRONG_STATE;
				g_snprintf(error_cause, 512, "Wrong state (register first)");
				goto error;
			}
			if(session->account.registration_status < janus_auvioussip_registration_status_registered) {
			...
			/* Unregister now */
			session->account.registration_status = janus_auvioussip_registration_status_unregistering;
			...
			if(session->status >= janus_auvioussip_call_status_inviting) {
				JANUS_LOG(LOG_ERR, "Wrong state (already in a call? status=%s)\n", janus_auvioussip_call_status_string(session->status));
				error_code = JANUS_AUVIOUSSIP_ERROR_WRONG_STATE;
				g_snprintf(error_cause, 512, "Wrong state (already in a call? status=%s)", janus_auvioussip_call_status_string(session->status));
				goto error;
			}
			...
			janus_auvioussip_uri_t target_uri;
			if(janus_auvioussip_parse_uri(&target_uri, uri_text) < 0) {
				JANUS_LOG(LOG_ERR, "Invalid user address %s\n", uri_text);
				error_code = JANUS_AUVIOUSSIP_ERROR_INVALID_ADDRESS;
				g_snprintf(error_cause, 512, "Invalid user address %s\n", uri_text);
				goto error;
			...
			if(!msg_sdp) {
				...
				error_code = JANUS_AUVIOUSSIP_ERROR_MISSING_SDP;
			...
			if(janus_auvioussip_allocate_local_ports(session) < 0) {
				JANUS_LOG(LOG_ERR, "Could not allocate RTP/RTCP ports\n");
				janus_sdp_destroy(parsed_sdp);
				error_code = JANUS_AUVIOUSSIP_ERROR_IO_ERROR;
				g_snprintf(error_cause, 512, "Could not allocate RTP/RTCP ports");
				goto error;
			}
			char *sdp = janus_auvioussip_sdp_manipulate(session, parsed_sdp, FALSE);
			if(sdp == NULL) {
			...
			g_atomic_int_set(&session->hangingup, 0);
			session->status = janus_auvioussip_call_status_inviting;
			char *callid;
			...
			if (request_callid) {
				...
				callid = g_malloc0(24);
				janus_auvioussip_random_string(24, callid);
			}
			/* Also notify event handlers */
			if(notify_events && gateway->events_is_enabled()) {
				json_t *info = json_object();
			...
			if(secret) {
				...
				session->account.secret_type = janus_auvioussip_secret_type_plaintext;
			} else if(ha1_secret) {
				...
				session->account.secret_type = janus_auvioussip_secret_type_hashed;
			}
			/* Send INVITE */
			g_free(session->callee);
			...
			if(session->status != janus_auvioussip_call_status_invited) {
				JANUS_LOG(LOG_ERR, "Wrong state (not invited? status=%s)\n", janus_auvioussip_call_status_string(session->status));
				error_code = JANUS_AUVIOUSSIP_ERROR_WRONG_STATE;
				g_snprintf(error_cause, 512, "Wrong state (not invited? status=%s)", janus_auvioussip_call_status_string(session->status));
				goto error;
			}
			...
			if(session->media.require_srtp && !session->media.has_srtp_remote) {
				...
				error_code = JANUS_AUVIOUSSIP_ERROR_TOO_STRICT;
			...
			char *sdp = janus_auvioussip_sdp_manipulate(session, parsed_sdp, TRUE);
			if(sdp == NULL) {
				JANUS_LOG(LOG_ERR, "Could not allocate RTP/RTCP ports\n");
				janus_sdp_destroy(parsed_sdp);
				error_code = JANUS_AUVIOUSSIP_ERROR_IO_ERROR;
			...
			if(answer) {
				...
				janus_refcount_increase(&session->ref);
				session->relayer_thread = g_thread_try_new(tname, janus_auvioussip_relay_thread, session, &error);
				if(error != NULL) {
			...
			if(!(session->status == janus_auvioussip_call_status_inviting || session->status == janus_auvioussip_call_status_incall)) {
			...
			char *sdp = janus_auvioussip_sdp_manipulate(session, parsed_sdp, !offer);
			if(sdp == NULL) {
				JANUS_LOG(LOG_ERR, "Error manipulating SDP\n");
				janus_sdp_destroy(parsed_sdp);
				error_code = JANUS_AUVIOUSSIP_ERROR_IO_ERROR;
			...
			if(!record_audio && !record_video && !record_peer_audio && !record_peer_video) {
				...
				error_code = JANUS_AUVIOUSSIP_ERROR_RECORDING_ERROR;
            ...
            gateway->send_pli(session->handle);
				...
				janus_auvioussip_recorder_close(session, record_audio, record_peer_audio, record_video, record_peer_video);
			}
			janus_mutex_unlock(&session->rec_mutex);
			/* Notify the result */
			result = json_object();
			...
			error_code = JANUS_AUVIOUSSIP_ERROR_INVALID_REQUEST;
		...
		if(result != NULL)
			json_object_set_new(event, "result", result);
		json_object_set_new(event, "call_id", json_string(session->callid));
		int ret = gateway->push_event(msg->handle, &janus_auvioussip_plugin, msg->transaction, event, NULL);
		JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (%s)\n", ret, janus_get_api_error(ret));
		json_decref(event);
		janus_auvioussip_message_free(msg);
		continue;
...
void janus_auvioussip_sofia_callback(nua_event_t event, int status, char const *phrase, nua_t *nua, nua_magic_t *magic, nua_handle_t *nh, nua_hmagic_t *hmagic, sip_t const *sip, tagi_t tags[])
{
	janus_auvioussip_session *session = (janus_auvioussip_session *)magic;
	...
	/* Status or Error Indications */
		...
		case nua_i_state:
			...
			if(callstate == nua_callstate_proceeding &&
				...
				int ret = gateway->push_event(session->handle, &janus_auvioussip_plugin, session->transaction, call, NULL);
				JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (%s)\n", ret, janus_get_api_error(ret));
				json_decref(call);
				/* Also notify event handlers */
				if(notify_events && gateway->events_is_enabled()) {
	...
	/* SIP requests */
		case nua_i_ack: {
			...
			if(sip->sip_payload && sip->sip_payload->pl_data) {
				...
				janus_auvioussip_sofia_callback(nua_r_invite, 700, "ACK", nua, magic, nh, hmagic, sip, tags);
			}
			break;
		}
		case nua_i_outbound:
		...
		case nua_i_invite: {
			...
			if(session->stack->s_nh_i == NULL) {
				...
				} else if(session->status >= janus_auvioussip_call_status_inviting) {
			...
			if(busy) {
				...
				int ret = gateway->push_event(session->handle, &janus_auvioussip_plugin, session->transaction, missed, NULL);
			...
			if(sdp) {
				...
				janus_auvioussip_sdp_process(session, sdp, FALSE, reinvite, &changed);
			...
			json_object_set_new(call, "result", calling);
			json_object_set_new(call, "call_id", json_string(session->callid));
			int ret = gateway->push_event(session->handle, &janus_auvioussip_plugin, session->transaction, call, jsep);
			JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
			json_decref(call);
			if(jsep)
				json_decref(jsep);
		...
		case nua_i_info: {
			...
			int ret = gateway->push_event(session->handle, &janus_auvioussip_plugin, session->transaction, info, NULL);
			JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
			json_decref(info);
			/* Send a 200 back */
			nua_respond(nh, 200, sip_status_phrase(200), TAG_END());
		...
		case nua_i_message: {
			...
			int ret = gateway->push_event(session->handle, &janus_auvioussip_plugin, session->transaction, message, NULL);
			JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
			json_decref(message);
			/* Send a 200 back */
			nua_respond(nh, 200, sip_status_phrase(200), TAG_END());
		...
		case nua_r_invite: {
			...
			if(status < 200) {
				...
				if(status == 180) {
					...
					int ret = gateway->push_event(session->handle, &janus_auvioussip_plugin, session->transaction, ringing, NULL);
					JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
					json_decref(ringing);
					break;
				} else if(status == 183) {
				...
				g_snprintf(auth, sizeof(auth), "%s%s:%s:%s:%s%s",
					session->account.secret_type == janus_auvioussip_secret_type_hashed ? "HA1+" : "",
					scheme,
					realm,
					authuser,
					session->account.secret_type == janus_auvioussip_secret_type_hashed ? "HA1+" : "",
			...
			janus_auvioussip_sdp_process(session, sdp, TRUE, update, &changed);
		...
		case nua_r_unregister: {
			...
			if(status == 200) {
				if(event == nua_r_register) {
					if(session->account.registration_status < janus_auvioussip_registration_status_registered)
						session->account.registration_status = janus_auvioussip_registration_status_registered;
				} else {
					session->account.registration_status = janus_auvioussip_registration_status_unregistered;
				}
				...
				int ret = gateway->push_event(session->handle, &janus_auvioussip_plugin, session->transaction, reg, NULL);
				JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
				json_decref(reg);
				/* Also notify event handlers */
				if(notify_events && gateway->events_is_enabled()) {
				...
				session->account.registration_status = janus_auvioussip_registration_status_failed;
				...
				json_object_set_new(event, "result", result);
				int ret = gateway->push_event(session->handle, &janus_auvioussip_plugin, session->transaction, event, NULL);
				JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
				json_decref(event);
				/* Also notify event handlers */
				if(notify_events && gateway->events_is_enabled()) {
				...
				if(!strcasecmp(a->name, "crypto")) {
					if(m->type == JANUS_SDP_AUDIO || m->type == JANUS_SDP_VIDEO) {
						...
						if(res != 3) {
							...
							janus_auvioussip_srtp_set_remote(session, video, profile, crypto);
	...
	while(temp) {
		...
		if(m->type == JANUS_SDP_AUDIO) {
			...
			if(session->media.has_srtp_local) {
				char *profile = NULL;
				char *crypto = NULL;
				janus_auvioussip_srtp_set_local(session, FALSE, &profile, &crypto);
			...
			if(session->media.has_srtp_local) {
				char *profile = NULL;
				char *crypto = NULL;
				janus_auvioussip_srtp_set_local(session, TRUE, &profile, &crypto);
				janus_sdp_attribute *a = janus_sdp_attribute_create("crypto", "1 %s inline:%s", profile, crypto);
				g_free(profile);
				g_free(crypto);
				m->attributes = g_list_append(m->attributes, a);
...
static void janus_auvioussip_connect_sockets(janus_auvioussip_session *session, struct sockaddr_in *server_addr) {
...
static void *janus_auvioussip_relay_thread(void *data) {
	janus_auvioussip_session *session = (janus_auvioussip_session *)data;
	if(!session) {
	...
	if(have_server_ip)
		janus_auvioussip_connect_sockets(session, &server_addr);
	...
	while(goon && session != NULL && !g_atomic_int_get(&session->destroyed) &&
			session->status > janus_auvioussip_call_status_idle &&
			session->status < janus_auvioussip_call_status_closing) {	/* FIXME We need a per-call watchdog as well */
		...
		for(i=0; i<num; i++) {
			if(fds[i].revents & (POLLERR | POLLHUP)) {
				...
				janus_auvioussip_hangup_media(session->handle);
				break;
...
/* Sofia Event thread */
gpointer janus_auvioussip_sofia_thread(gpointer user_data) {
	janus_auvioussip_session *session = (janus_auvioussip_session *)user_data;
	if(session == NULL) {
		g_thread_unref(g_thread_self());
		return NULL;
	}
	...
	session->stack->s_nua = nua_create(session->stack->s_root,
				janus_auvioussip_sofia_callback,
				session,
