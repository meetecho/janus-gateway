...
typedef struct janus_audiobridge_room {
    ...
    /* Room description */
    gchar *room_secret;
    /* Secret needed to manipulate (e.g., destroy) this room */
    uint32_t sampling_rate;
    /* Sampling rate of the mix (e.g., 16000 for wideband) */
    gboolean record;
    /* Whether this room has to be recorded or not */
    gchar *record_file;
    /* Path of the recording file */
    FILE *recording;
    /* File to record the room into */
    gboolean destroy;
    /* Value to flag the room for destruction */
...
typedef struct janus_audiobridge_participant {
    ...
    janus_audiobridge_room *room;
    /* Room */
    guint64 user_id;
    /* Unique ID in the room */
    gchar *display;
    ...
    if (config != NULL) {
        ...
        while (cat != NULL) {
            ...
            if (audiobridge->sampling_rate != 16000) {
                JANUS_LOG(LOG_ERR, "We currently only support 16kHz (wideband) as a sampling rate for audio rooms, changing %"
                        SCNu32
            ...
            JANUS_LOG(LOG_VERB, "Created audiobridge: %"
                    SCNu64
...
static void *janus_audiobridge_handler(void *data) {
    ...
    while (initialized && !stopping) {
        ...
        if (!strcasecmp(request_text, "create")) {
            ...
            JANUS_LOG(LOG_INFO, "Creating a new audiobridge\n");
            ...
            if (sampling)
                ...
                audiobridge->sampling_rate = 16000;
            if (audiobridge->sampling_rate != 16000) {
                JANUS_LOG(LOG_WARN, "We currently only support 16kHz (wideband) as a sampling rate for audio rooms, changing %"
                        SCNu32
                        " to 16000...\n", audiobridge->sampling_rate);
                audiobridge->sampling_rate = 16000;
            }
            ...
            JANUS_LOG(LOG_VERB, "  -- Participant ID: %"
                    SCNu64
                    "\n", user_id);
        ...
        if (!msg->sdp) {
            ...
            JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (took %"
                    SCNu64
...
static void *janus_audiobridge_mixer_thread(void *data) {
    ...
    JANUS_LOG(LOG_VERB, "Thread is for mixing room %"
            SCNu64
    ...
    JANUS_LOG(LOG_VERB, "Leaving mixer thread for room %"
            SCNu64
            " (%s)...\n", audiobridge->room_id, audiobridge->room_name);
    return NULL;
}
