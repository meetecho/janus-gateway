		...
		if(pkt->seq_number == participant->expected_seq) {
			...
			if(participant->fec && gap < DEFAULT_PREBUFFERING) {
				...
				for(i=1; i<=gap ; i++) {
					...
					lost_pkt->ssrc = 0;
					lost_pkt->timestamp = participant->last_timestamp + (i * AUDIO_STEP_TS);
...
static void *janus_audiobridge_mixer_thread(void *data) {
	...
	opus_int32 buffer[AUDIO_STEP_TS], sumBuffer[AUDIO_STEP_TS];
	opus_int16 outBuffer[AUDIO_STEP_TS], *curBuffer = NULL;
	memset(buffer, 0, AUDIO_STEP_TS*4);
	memset(sumBuffer, 0, AUDIO_STEP_TS*4);
	memset(outBuffer, 0, AUDIO_STEP_TS*2);
			...
			if(go_on) {
				...
				if(length < 0) {
					...
					while(audiobridge->rtp_udp_sock > 0 && g_hash_table_iter_next(&iter, &key, &value)) {
						...
						forwarder->timestamp += AUDIO_STEP_TS;
...
static void janus_audiobridge_relay_rtp_packet(gpointer data, gpointer user_data) {
	...
	janus_rtp_header_update(packet->data, &participant->context, FALSE, AUDIO_STEP_TS);
	if(gateway != NULL)
