...
static struct janus_json_parameter room_parameters[] = {
	{"room", JSON_INTEGER, JANUS_JSON_PARAM_POSITIVE | JANUS_JSON_PARAM_POSITIVE}
};
...
static json_t *janus_audiobridge_process_synchronous_request(janus_audiobridge_session *session, json_t *message) {
	...
	if(!strcasecmp(request_text, "create")) {
		...
		JANUS_LOG(LOG_VERB, "Muted user %s from room %s\n", user_id_str, room_id_str);
...
static void *janus_audiobridge_mixer_thread(void *data) {
	...
	while(!g_atomic_int_get(&stopping) && !g_atomic_int_get(&audiobridge->destroyed)) {
		...
		if((count+rf_count) == 0) {
			...
			if(prev_count > 0) {
				JANUS_LOG(LOG_VERB, "Last user/forwarder just left room %s, going idle...\n", audiobridge->room_id_str);
				prev_count = 0;
			}
			continue;
		}
		if(prev_count == 0) {
			JANUS_LOG(LOG_VERB, "First user/forwarder just joined room %s, waking it up...\n", audiobridge->room_id_str);
		}
