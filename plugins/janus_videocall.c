...
void janus_videocall_incoming_data(janus_plugin_session *handle, char *label, char *protocol, gboolean textdata, char *buf, int len);
...
void janus_videocall_incoming_data(janus_plugin_session *handle, char *label, char *protocol, gboolean textdata, char *buf, int len) {
	...
	if(gateway) {
		...
		/* Forward the packet to the peer */
		gateway->relay_data(peer->handle, label, protocol, textdata, buf, len);
	}
}
