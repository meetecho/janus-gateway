 ...
 * as described in \ref JS. In case updates to the session are needed (currently
 * only for the purpose of ICE restarts), plugins have to add a boolean
 * property called \c update to the JSEP object: the core does the same
 * whenever it's the user requesting a renegotiation.
 ...
 * In next versions of Janus we may work on stuff like plugins "chaining": that
