			...
			if(sps_dec && pps_dec) {
				...
				*nal = 0x18;
				...
				if(!source->rtsp) {
					...
					source->h264_spspps = NULL;
					source->h264_spspps_len = 0;
					g_free(buf);
				}
			}
			g_free(sps_dec);
...
static int janus_streaming_rtsp_connect_to_server(janus_streaming_mountpoint *mp) {
	...
	/* The mountpoint URL comes straight from the plugin API: restrict libcurl
	 * to RTSP so it cannot be pointed at gopher://, file://, ftp://, ... */
#if CURL_AT_LEAST_VERSION(7, 85, 0)
	curl_easy_setopt(curl, CURLOPT_PROTOCOLS_STR, "rtsp,rtsps");
#else
