...
static void *janus_streaming_relay_thread(void *data) {
  ...
  int resfd = 0, bytes = 0;
  int v_last_block = 0, a_last_block = 0; // indicates last received byte is last packet for each sampling.
  struct pollfd fds[2];
  char buffer[1500];
  ...
  while(!g_atomic_int_get(&stopping) && !mountpoint->destroyed) {
    ...
    if(video_fd && (fds[1].revents & POLLIN)) {
      ...
      if(mountpoint->active == FALSE)
        mountpoint->active = TRUE;
      addrlen = sizeof(remote);
      bytes = recvfrom(video_fd, buffer, 65535, 0, (struct sockaddr*)&remote, &addrlen);
