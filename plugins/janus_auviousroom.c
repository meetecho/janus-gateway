/*! \file   janus_auviousroom.c
 * \author Lorenzo Miniero <lorenzo@meetecho.com>
 * \copyright GNU General Public License v3
 * \brief  Janus AuviousRoom plugin
 ...
 * A pre-filled configuration file is provided in \c conf/janus.plugin.auviousroom.cfg
...
notify_joining = true|false (optional, whether to notify all participants when a new
            participant joins the room. The Auviousroom plugin by design only notifies
...
int janus_auviousroom_init(janus_callbacks *callback, const char *config_path);
void janus_auviousroom_destroy(void);
int janus_auviousroom_get_api_compatibility(void);
int janus_auviousroom_get_version(void);
const char *janus_auviousroom_get_version_string(void);
const char *janus_auviousroom_get_description(void);
const char *janus_auviousroom_get_name(void);
const char *janus_auviousroom_get_author(void);
const char *janus_auviousroom_get_package(void);
void janus_auviousroom_create_session(janus_plugin_session *handle, int *error);
struct janus_plugin_result *janus_auviousroom_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep);
void janus_auviousroom_setup_media(janus_plugin_session *handle);
void janus_auviousroom_incoming_rtp(janus_plugin_session *handle, janus_plugin_rtp *packet);
void janus_auviousroom_incoming_rtcp(janus_plugin_session *handle, janus_plugin_rtcp *packet);
void janus_auviousroom_incoming_data(janus_plugin_session *handle, janus_plugin_data *packet);
...
json_t *janus_auviousroom_query_session(janus_plugin_session *handle);

/* Plugin setup */
static janus_plugin janus_auviousroom_plugin =
	JANUS_PLUGIN_INIT (
		.init = janus_auviousroom_init,
		.destroy = janus_auviousroom_destroy,

		.get_api_compatibility = janus_auviousroom_get_api_compatibility,
		.get_version = janus_auviousroom_get_version,
		.get_version_string = janus_auviousroom_get_version_string,
		.get_description = janus_auviousroom_get_description,
		.get_name = janus_auviousroom_get_name,
		.get_author = janus_auviousroom_get_author,
		.get_package = janus_auviousroom_get_package,
		
		.create_session = janus_auviousroom_create_session,
		.handle_message = janus_auviousroom_handle_message,
		.setup_media = janus_auviousroom_setup_media,
		.incoming_rtp = janus_auviousroom_incoming_rtp,
		.incoming_rtcp = janus_auviousroom_incoming_rtcp,
		.incoming_data = janus_auviousroom_incoming_data,
		.slow_link = janus_auviousroom_slow_link,
		.hangup_media = janus_auviousroom_hangup_media,
		.destroy_session = janus_auviousroom_destroy_session,
		.query_session = janus_auviousroom_query_session,
	);

/* Plugin creator */
janus_plugin *create(void) {
	JANUS_LOG(LOG_VERB, "%s created!\n", JANUS_AUVIOUSROOM_NAME);
	return &janus_auviousroom_plugin;
}
...
static void *janus_auviousroom_handler(void *data);
static void janus_auviousroom_relay_rtp_packet(gpointer data, gpointer user_data);
static void janus_auviousroom_relay_data_packet(gpointer data, gpointer user_data);

typedef enum janus_auviousroom_p_type {
	janus_auviousroom_p_type_none = 0,
	janus_auviousroom_p_type_subscriber,			/* Generic listener/subscriber */
	janus_auviousroom_p_type_publisher,			/* Participant/publisher */
} janus_auviousroom_p_type;

typedef struct janus_auviousroom_message {
	janus_plugin_session *handle;
	char *transaction;
	json_t *message;
	json_t *jsep;
} janus_auviousroom_message;
static GAsyncQueue *messages = NULL;
static janus_auviousroom_message exit_message;

static void janus_auviousroom_message_free(janus_auviousroom_message *msg) {
...
typedef enum janus_auviousroom_audiocodec {
	JANUS_AUVIOUSROOM_NOAUDIO,	/* Publishers won't do any audio */
	JANUS_AUVIOUSROOM_OPUS,		/* Publishers will have to/may use OPUS */
	JANUS_AUVIOUSROOM_PCMU,		/* Publishers will have to/may use PCMU 8K */
	JANUS_AUVIOUSROOM_PCMA,		/* Publishers will have to/may use PCMA 8K */
	JANUS_AUVIOUSROOM_G722,		/* Publishers will have to/may use G.722 */
	JANUS_AUVIOUSROOM_ISAC_32K,	/* Publishers will have to/may use ISAC 32K */
	JANUS_AUVIOUSROOM_ISAC_16K	/* Publishers will have to/may use ISAC 16K */
} janus_auviousroom_audiocodec;
static const char *janus_auviousroom_audiocodec_name(janus_auviousroom_audiocodec acodec) {
	switch(acodec) {
		case JANUS_AUVIOUSROOM_NOAUDIO:
			return "none";
		case JANUS_AUVIOUSROOM_OPUS:
			return "opus";
		case JANUS_AUVIOUSROOM_PCMU:
			return "pcmu";
		case JANUS_AUVIOUSROOM_PCMA:
			return "pcma";
		case JANUS_AUVIOUSROOM_G722:
			return "g722";
		case JANUS_AUVIOUSROOM_ISAC_32K:
			return "isac32";
		case JANUS_AUVIOUSROOM_ISAC_16K:
			return "isac16";
		default:
			/* Shouldn't happen */
			return "opus";
...
static janus_auviousroom_audiocodec janus_auviousroom_audiocodec_from_name(const char *name) {
	if(name == NULL)
		return JANUS_AUVIOUSROOM_NOAUDIO;
	else if(!strcasecmp(name, "opus"))
		return JANUS_AUVIOUSROOM_OPUS;
	else if(!strcasecmp(name, "isac32"))
		return JANUS_AUVIOUSROOM_ISAC_32K;
	else if(!strcasecmp(name, "isac16"))
		return JANUS_AUVIOUSROOM_ISAC_16K;
	else if(!strcasecmp(name, "pcmu"))
		return JANUS_AUVIOUSROOM_PCMU;
	else if(!strcasecmp(name, "pcma"))
		return JANUS_AUVIOUSROOM_PCMA;
	else if(!strcasecmp(name, "g722"))
		return JANUS_AUVIOUSROOM_G722;
	JANUS_LOG(LOG_WARN, "Unsupported audio codec '%s'\n", name);
	return JANUS_AUVIOUSROOM_NOAUDIO;
}
static int janus_auviousroom_audiocodec_pt(janus_auviousroom_audiocodec acodec) {
	switch(acodec) {
		case JANUS_AUVIOUSROOM_NOAUDIO:
			return -1;
		case JANUS_AUVIOUSROOM_OPUS:
...
typedef enum janus_auviousroom_videocodec {
	JANUS_AUVIOUSROOM_NOVIDEO,	/* Publishers won't do any audio */
	JANUS_AUVIOUSROOM_VP8,		/* Publishers will have to/may use VP8 */
	JANUS_AUVIOUSROOM_VP9,		/* Publishers will have to/may use VP9 */
	JANUS_AUVIOUSROOM_H264		/* Publishers will have to/may use H264 */
} janus_auviousroom_videocodec;
static const char *janus_auviousroom_videocodec_name(janus_auviousroom_videocodec vcodec) {
	switch(vcodec) {
		case JANUS_AUVIOUSROOM_NOVIDEO:
			return "none";
		case JANUS_AUVIOUSROOM_VP8:
			return "vp8";
		case JANUS_AUVIOUSROOM_VP9:
			return "vp9";
		case JANUS_AUVIOUSROOM_H264:
			return "h264";
		default:
			/* Shouldn't happen */
			return "vp8";
...
static janus_auviousroom_videocodec janus_auviousroom_videocodec_from_name(const char *name) {
	if(name == NULL)
		return JANUS_AUVIOUSROOM_NOVIDEO;
	else if(!strcasecmp(name, "vp8"))
		return JANUS_AUVIOUSROOM_VP8;
	else if(!strcasecmp(name, "vp9"))
		return JANUS_AUVIOUSROOM_VP9;
	else if(!strcasecmp(name, "h264"))
		return JANUS_AUVIOUSROOM_H264;
	JANUS_LOG(LOG_WARN, "Unsupported video codec '%s'\n", name);
	return JANUS_AUVIOUSROOM_NOVIDEO;
}
static int janus_auviousroom_videocodec_pt(janus_auviousroom_videocodec vcodec) {
	switch(vcodec) {
		case JANUS_AUVIOUSROOM_NOVIDEO:
			return -1;
		case JANUS_AUVIOUSROOM_VP8:
...
typedef struct janus_auviousroom {
	...
	janus_auviousroom_audiocodec acodec[3];	/* Audio codec(s) to force on publishers */
	janus_auviousroom_videocodec vcodec[3];	/* Video codec(s) to force on publishers */
...
static void janus_auviousroom_free(janus_auviousroom *room);

typedef struct janus_auviousroom_session {
	janus_plugin_session *handle;
	janus_auviousroom_p_type participant_type;
	gpointer participant;
	gboolean started;
	gboolean stopping;
	volatile gint hangingup;
	gint64 destroyed;	/* Time at which this session was marked as destroyed */
} janus_auviousroom_session;
static GHashTable *sessions;
static GList *old_sessions;
static janus_mutex sessions_mutex = JANUS_MUTEX_INITIALIZER;

/* A host whose ports gets streamed RTP packets of the corresponding type */
typedef struct janus_auviousroom_rtp_forwarder {
	gboolean is_video;
	gboolean is_data;
	uint32_t ssrc;
	int payload_type;
...
} janus_auviousroom_rtp_forwarder;

typedef struct janus_auviousroom_participant {
	janus_auviousroom_session *session;
	janus_auviousroom *room;	/* Room */
...
} janus_auviousroom_participant;
static void janus_auviousroom_participant_free(janus_auviousroom_participant *p);
static void janus_auviousroom_rtp_forwarder_free_helper(gpointer data);
static guint32 janus_auviousroom_rtp_forwarder_add_helper(janus_auviousroom_participant *p,
	const gchar* host, int port, int pt, uint32_t ssrc, int substream, gboolean is_video, gboolean is_data);

typedef struct janus_auviousroom_listener {
	janus_auviousroom_session *session;
	janus_auviousroom *room;	/* Room */
	janus_auviousroom_participant *feed;	/* Participant this listener is subscribed to */
...
typedef struct janus_auviousroom_rtp_relay_packet {
...
static void janus_auviousroom_reqpli(janus_auviousroom_participant *publisher, const char *reason) {
  if(publisher == NULL)
    return;
  /* Send a PLI */
  JANUS_LOG(LOG_VERB, "%s sending PLI to %ld (%s)\n", reason,
...
/* Error codes */
#define JANUS_AUVIOUSROOM_ERROR_UNKNOWN_ERROR		499
#define JANUS_AUVIOUSROOM_ERROR_NO_MESSAGE		421
#define JANUS_AUVIOUSROOM_ERROR_INVALID_JSON		422
#define JANUS_AUVIOUSROOM_ERROR_INVALID_REQUEST	423
#define JANUS_AUVIOUSROOM_ERROR_JOIN_FIRST		424
#define JANUS_AUVIOUSROOM_ERROR_ALREADY_JOINED	425
#define JANUS_AUVIOUSROOM_ERROR_NO_SUCH_ROOM		426
#define JANUS_AUVIOUSROOM_ERROR_ROOM_EXISTS		427
#define JANUS_AUVIOUSROOM_ERROR_NO_SUCH_FEED		428
#define JANUS_AUVIOUSROOM_ERROR_MISSING_ELEMENT	429
#define JANUS_AUVIOUSROOM_ERROR_INVALID_ELEMENT	430
#define JANUS_AUVIOUSROOM_ERROR_INVALID_SDP_TYPE	431
#define JANUS_AUVIOUSROOM_ERROR_PUBLISHERS_FULL	432
#define JANUS_AUVIOUSROOM_ERROR_UNAUTHORIZED		433
#define JANUS_AUVIOUSROOM_ERROR_ALREADY_PUBLISHED	434
#define JANUS_AUVIOUSROOM_ERROR_NOT_PUBLISHED		435
#define JANUS_AUVIOUSROOM_ERROR_ID_EXISTS			436
#define JANUS_AUVIOUSROOM_ERROR_INVALID_SDP		437
...
static guint32 janus_auviousroom_rtp_forwarder_add_helper(janus_auviousroom_participant *p,
	...
	janus_auviousroom_rtp_forwarder *forward = g_malloc0(sizeof(janus_auviousroom_rtp_forwarder));
...
static void session_free(gpointer data) {
	if(data) {
		janus_auviousroom_session* session = (janus_auviousroom_session*)data;
		switch(session->participant_type) {
		case janus_auviousroom_p_type_publisher: 
			janus_auviousroom_participant_free(session->participant);
			break;   
		case janus_auviousroom_p_type_subscriber:
			janus_auviousroom_listener_free(session->participant);
			break;
		default:
			break;
		}
...
static void janus_auviousroom_rtp_forwarder_free_helper(gpointer data) {
	if(data) {
		janus_auviousroom_rtp_forwarder* forward = (janus_auviousroom_rtp_forwarder*)data;
		if(forward) {
			g_free(forward);
			forward = NULL;
		}
...
/* AuviousRoom watchdog/garbage collector (sort of) */
static void *janus_auviousroom_watchdog(void *data) {
	JANUS_LOG(LOG_INFO, "AuviousRoom watchdog started\n");
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(old_sessions != NULL) {
			...
			JANUS_LOG(LOG_HUGE, "Checking %d old AuviousRoom sessions...\n", g_list_length(old_sessions));
			while(sl) {
				janus_auviousroom_session *session = (janus_auviousroom_session *)sl->data;
				...
				if(now-session->destroyed >= 5*G_USEC_PER_SEC) {
					...
					JANUS_LOG(LOG_VERB, "Freeing old AuviousRoom session\n");
					GList *rm = sl->next;
		...
		if(old_rooms != NULL) {
			...
			while(rl) {
				janus_auviousroom* room = (janus_auviousroom*)rl->data;
				if(!initialized || stopping){
					break;
				}
				if(!room) {
	...
	JANUS_LOG(LOG_INFO, "AuviousRoom watchdog stopped\n");
	return NULL;
}
...
int janus_auviousroom_init(janus_callbacks *callback, const char *config_path) {
	...
	/* Read configuration */
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.cfg", config_path, JANUS_AUVIOUSROOM_PACKAGE);
	...
	rooms = g_hash_table_new_full(g_int64_hash, g_int64_equal,
		(GDestroyNotify)g_free, (GDestroyNotify) janus_auviousroom_free);
	sessions = g_hash_table_new(NULL, NULL);

	messages = g_async_queue_new_full((GDestroyNotify) janus_auviousroom_message_free);
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			...
			auviousroom->room_name = description;
			if(secret != NULL && secret->value != NULL) {
				auviousroom->room_secret = g_strdup(secret->value);
			}
			if(pin != NULL && pin->value != NULL) {
				auviousroom->room_pin = g_strdup(pin->value);
			}
			auviousroom->is_private = priv && priv->value && janus_is_true(priv->value);
			auviousroom->require_pvtid = req_pvtid && req_pvtid->value && janus_is_true(req_pvtid->value);
			auviousroom->max_publishers = 3;	/* FIXME How should we choose a default? */
			if(maxp != NULL && maxp->value != NULL)
				auviousroom->max_publishers = atol(maxp->value);
			if(auviousroom->max_publishers < 0)
				auviousroom->max_publishers = 3;	/* FIXME How should we choose a default? */
			auviousroom->bitrate = 0;
			if(bitrate != NULL && bitrate->value != NULL)
				auviousroom->bitrate = atol(bitrate->value);
			if(auviousroom->bitrate > 0 && auviousroom->bitrate < 64000)
				auviousroom->bitrate = 64000;	/* Don't go below 64k */
			auviousroom->fir_freq = 0;
			if(firfreq != NULL && firfreq->value != NULL)
				auviousroom->fir_freq = atol(firfreq->value);
			/* By default, we force Opus as the only audio codec */
			auviousroom->acodec[0] = JANUS_AUVIOUSROOM_OPUS;
			auviousroom->acodec[1] = JANUS_AUVIOUSROOM_NOAUDIO;
			auviousroom->acodec[2] = JANUS_AUVIOUSROOM_NOAUDIO;
			...
			if(audiocodec && audiocodec->value) {
				...
				if(codec != NULL) {
					...
					while(codec != NULL) {
						...
						if(strlen(codec) > 0)
							auviousroom->acodec[i] = janus_auviousroom_audiocodec_from_name(codec);
						i++;
						codec = list[i];
					}
				}
			...
			auviousroom->vcodec[0] = JANUS_AUVIOUSROOM_VP8;
			auviousroom->vcodec[1] = JANUS_AUVIOUSROOM_NOVIDEO;
			auviousroom->vcodec[2] = JANUS_AUVIOUSROOM_NOVIDEO;
			/* Check if we're forcing a different single codec, or allowing more than one */
			if(videocodec && videocodec->value) {
				...
				if(codec != NULL) {
					...
					while(codec != NULL) {
						...
						if(strlen(codec) > 0)
							auviousroom->vcodec[i] = janus_auviousroom_videocodec_from_name(codec);
						i++;
						codec = list[i];
					}
				}
			...
			if(svc && svc->value && janus_is_true(svc->value)) {
				if(auviousroom->vcodec[0] == JANUS_AUVIOUSROOM_VP9 &&
						auviousroom->vcodec[1] == JANUS_AUVIOUSROOM_NOVIDEO &&
						auviousroom->vcodec[2] == JANUS_AUVIOUSROOM_NOVIDEO) {
					auviousroom->do_svc = TRUE;
				} else {
					JANUS_LOG(LOG_WARN, "SVC is only supported, in an experimental way, for VP9 only rooms: disabling it...\n");
				}
			}
			auviousroom->audiolevel_ext = TRUE;
			if(audiolevel_ext != NULL && audiolevel_ext->value != NULL)
				auviousroom->audiolevel_ext = janus_is_true(audiolevel_ext->value);
			auviousroom->audiolevel_event = FALSE;
			if(audiolevel_event != NULL && audiolevel_event->value != NULL)
				auviousroom->audiolevel_event = janus_is_true(audiolevel_event->value);
			if(auviousroom->audiolevel_event) {
				auviousroom->audio_active_packets = 100;
				if(audio_active_packets != NULL && audio_active_packets->value != NULL){
					if(atoi(audio_active_packets->value) > 0) {
						auviousroom->audio_active_packets = atoi(audio_active_packets->value);
					} else {
						JANUS_LOG(LOG_WARN, "Invalid audio_active_packets value, using default: %d\n", auviousroom->audio_active_packets);
					}
				}
				auviousroom->audio_level_average = 25;
				if(audio_level_average != NULL && audio_level_average->value != NULL) {
					if(atoi(audio_level_average->value) > 0) {
						auviousroom->audio_level_average = atoi(audio_level_average->value);
					} else {
						JANUS_LOG(LOG_WARN, "Invalid audio_level_average value provided, using default: %d\n", auviousroom->audio_level_average);
					}
				}
			}
			auviousroom->videoorient_ext = TRUE;
			if(videoorient_ext != NULL && videoorient_ext->value != NULL)
				auviousroom->videoorient_ext = janus_is_true(videoorient_ext->value);
			auviousroom->playoutdelay_ext = TRUE;
			if(playoutdelay_ext != NULL && playoutdelay_ext->value != NULL)
				auviousroom->playoutdelay_ext = janus_is_true(playoutdelay_ext->value);
			if(record && record->value) {
				auviousroom->record = janus_is_true(record->value);
			}
			if(rec_dir && rec_dir->value) {
				auviousroom->rec_dir = g_strdup(rec_dir->value);
			}
			/* By default, the auviousroom plugin does not notify about participants simply joining the room.
			   It only notifies when the participant actually starts publishing media. */
			auviousroom->notify_joining = FALSE;
			if(notify_joining != NULL && notify_joining->value != NULL)
				auviousroom->notify_joining = janus_is_true(notify_joining->value);
			auviousroom->destroyed = 0;
			janus_mutex_init(&auviousroom->participants_mutex);
			auviousroom->participants = g_hash_table_new_full(g_int64_hash, g_int64_equal, (GDestroyNotify)g_free, NULL);
			auviousroom->private_ids = g_hash_table_new(NULL, NULL);
			auviousroom->check_tokens = FALSE;	/* Static rooms can't have an "allowed" list yet, no hooks to the configuration file */
			auviousroom->allowed = g_hash_table_new_full(g_str_hash, g_str_equal, (GDestroyNotify)g_free, NULL);
			janus_mutex_lock(&rooms_mutex);
			g_hash_table_insert(rooms, janus_uint64_dup(auviousroom->room_id), auviousroom);
			janus_mutex_unlock(&rooms_mutex);
			...
			g_snprintf(audio_codecs, sizeof(audio_codecs), "%s", janus_auviousroom_audiocodec_name(auviousroom->acodec[0]));
			if(auviousroom->acodec[1] != JANUS_AUVIOUSROOM_NOAUDIO) {
				g_strlcat(audio_codecs, "|", sizeof(audio_codecs));
				g_strlcat(audio_codecs, janus_auviousroom_audiocodec_name(auviousroom->acodec[1]), sizeof(audio_codecs));
			}
			if(auviousroom->acodec[2] != JANUS_AUVIOUSROOM_NOAUDIO) {
				g_strlcat(audio_codecs, "|", sizeof(audio_codecs));
			...
			if(auviousroom->vcodec[1] != JANUS_AUVIOUSROOM_NOVIDEO) {
				g_strlcat(video_codecs, "|", sizeof(video_codecs));
				g_strlcat(video_codecs, janus_auviousroom_videocodec_name(auviousroom->vcodec[1]), sizeof(video_codecs));
			}
			if(auviousroom->vcodec[2] != JANUS_AUVIOUSROOM_NOVIDEO) {
				g_strlcat(video_codecs, "|", sizeof(video_codecs));
				g_strlcat(video_codecs, janus_auviousroom_videocodec_name(auviousroom->vcodec[2]), sizeof(video_codecs));
			}
			JANUS_LOG(LOG_VERB, "Created auviousroom: %"SCNu64" (%s, %s, %s/%s codecs, secret: %s, pin: %s, pvtid: %s)\n",
				auviousroom->room_id, auviousroom->room_name,
				auviousroom->is_private ? "private" : "public",
				audio_codecs, video_codecs,
				auviousroom->room_secret ? auviousroom->room_secret : "no secret",
				auviousroom->room_pin ? auviousroom->room_pin : "no pin",
				auviousroom->require_pvtid ? "required" : "optional");
			if(auviousroom->record) {
				JANUS_LOG(LOG_VERB, "  -- Room is going to be recorded in %s\n", auviousroom->rec_dir ? auviousroom->rec_dir : "the current folder");
			}
			cl = cl->next;
		}
	...
	while (g_hash_table_iter_next(&iter, NULL, &value)) {
		janus_auviousroom *vr = value;
		/* Compute a list of the supported codecs for the summary */
		char audio_codecs[100], video_codecs[100];
		memset(audio_codecs, 0, sizeof(audio_codecs));
		g_snprintf(audio_codecs, sizeof(audio_codecs), "%s", janus_auviousroom_audiocodec_name(vr->acodec[0]));
		if(vr->acodec[1] != JANUS_AUVIOUSROOM_NOAUDIO) {
			g_strlcat(audio_codecs, "|", sizeof(audio_codecs));
			g_strlcat(audio_codecs, janus_auviousroom_audiocodec_name(vr->acodec[1]), sizeof(audio_codecs));
		}
		if(vr->acodec[2] != JANUS_AUVIOUSROOM_NOAUDIO) {
			g_strlcat(audio_codecs, "|", sizeof(audio_codecs));
			g_strlcat(audio_codecs, janus_auviousroom_audiocodec_name(vr->acodec[2]), sizeof(audio_codecs));
		}
		memset(video_codecs, 0, sizeof(video_codecs));
		g_snprintf(video_codecs, sizeof(video_codecs), "%s", janus_auviousroom_videocodec_name(vr->vcodec[0]));
		if(vr->vcodec[1] != JANUS_AUVIOUSROOM_NOVIDEO) {
			g_strlcat(video_codecs, "|", sizeof(video_codecs));
			g_strlcat(video_codecs, janus_auviousroom_videocodec_name(vr->vcodec[1]), sizeof(video_codecs));
		}
		if(vr->vcodec[2] != JANUS_AUVIOUSROOM_NOVIDEO) {
			g_strlcat(video_codecs, "|", sizeof(video_codecs));
			g_strlcat(video_codecs, janus_auviousroom_videocodec_name(vr->vcodec[2]), sizeof(video_codecs));
		}
	...
	watchdog = g_thread_try_new("auviousroom watchdog", &janus_auviousroom_watchdog, NULL, &error);
	if(error != NULL) {
		g_atomic_int_set(&initialized, 0);
		JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the AuviousRoom watchdog thread...\n", error->code, error->message ? error->message : "??");
		janus_config_destroy(config);
		return -1;
	}
	/* Launch the thread that will handle incoming messages */
	handler_thread = g_thread_try_new("auviousroom handler", janus_auviousroom_handler, NULL, &error);
	if(error != NULL) {
		g_atomic_int_set(&initialized, 0);
		JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the AuviousRoom handler thread...\n", error->code, error->message ? error->message : "??");
		janus_config_destroy(config);
		return -1;
	}
...
int janus_auviousroom_get_version(void) {
	return JANUS_AUVIOUSROOM_VERSION;
}

const char *janus_auviousroom_get_version_string(void) {
	return JANUS_AUVIOUSROOM_VERSION_STRING;
}

const char *janus_auviousroom_get_description(void) {
	return JANUS_AUVIOUSROOM_DESCRIPTION;
}

const char *janus_auviousroom_get_name(void) {
	return JANUS_AUVIOUSROOM_NAME;
}

const char *janus_auviousroom_get_author(void) {
	return JANUS_AUVIOUSROOM_AUTHOR;
}

const char *janus_auviousroom_get_package(void) {
	return JANUS_AUVIOUSROOM_PACKAGE;
}
...
void janus_auviousroom_create_session(janus_plugin_session *handle, int *error) {
	...
	janus_auviousroom_session *session = (janus_auviousroom_session *)g_malloc0(sizeof(janus_auviousroom_session));
	session->handle = handle;
	session->participant_type = janus_auviousroom_p_type_none;
...
static void janus_auviousroom_notify_participants(janus_auviousroom_participant *participant, json_t *msg) {
	...
	while (!participant->room->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
		janus_auviousroom_participant *p = value;
		if(p && p->session && p != participant) {
			JANUS_LOG(LOG_VERB, "Notifying participant %"SCNu64" (%s)\n", p->user_id, p->display ? p->display : "??");
			int ret = gateway->push_event(p->session->handle, &janus_auviousroom_plugin, NULL, msg, NULL);
			JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
		}
	}
}

static void janus_auviousroom_participant_joining(janus_auviousroom_participant *p) {
	...
	if(p->room && !p->room->destroyed && p->room->notify_joining) {
		...
		json_object_set_new(event, "auviousroom", json_string("event"));
		...
		janus_auviousroom_notify_participants(p, event);
...
static void janus_auviousroom_leave_or_unpublish(janus_auviousroom_participant *participant, gboolean is_leaving, gboolean kicked) {
	/* we need to check if the room still exists, may have been destroyed already */
	if(participant->room && !participant->room->destroyed) {
		...
		janus_auviousroom_notify_participants(participant, event);
		if(is_leaving) {
...
void janus_auviousroom_destroy_session(janus_plugin_session *handle, int *error) {
	...
	janus_auviousroom_session *session = (janus_auviousroom_session *)handle->plugin_handle; 
	if(!session) {
		JANUS_LOG(LOG_ERR, "No AuviousRoom session associated with this handle...\n");
		*error = -2;
		return;
	}
	if(session->destroyed) {
		JANUS_LOG(LOG_WARN, "AuviousRoom session already marked as destroyed...\n");
		return;
	}
	JANUS_LOG(LOG_VERB, "Removing AuviousRoom session...\n");
	/* Cleaning up and removing the session is done in a lazy way */
	janus_mutex_lock(&sessions_mutex);
	if(!session->destroyed) {
		/* Any related WebRTC PeerConnection is not available anymore either */
		janus_auviousroom_hangup_media(handle);
		session->destroyed = janus_get_monotonic_time();
		old_sessions = g_list_append(old_sessions, session);
		if(session->participant_type == janus_auviousroom_p_type_publisher) {
			...
			participant->acodec = JANUS_AUVIOUSROOM_NOAUDIO;
			participant->vcodec = JANUS_AUVIOUSROOM_NOVIDEO;
			...
			// session->participant_type = janus_auviousroom_p_type_none;
			janus_auviousroom_leave_or_unpublish(participant, TRUE, FALSE);
		} else if(session->participant_type == janus_auviousroom_p_type_subscriber) {
...
json_t *janus_auviousroom_query_session(janus_plugin_session *handle) {
	...
	if(session->participant) {
		if(session->participant_type == janus_auviousroom_p_type_none) {
			json_object_set_new(info, "type", json_string("none"));
		} else if(session->participant_type == janus_auviousroom_p_type_publisher) {
			json_object_set_new(info, "type", json_string("publisher"));
			janus_auviousroom_participant *participant = (janus_auviousroom_participant *)session->participant;
			if(participant) {
				janus_auviousroom *room = participant->room; 
				...
				if(participant->audio)
					json_object_set_new(media, "audio_codec", json_string(janus_auviousroom_audiocodec_name(participant->acodec)));
				json_object_set_new(media, "video", participant->video ? json_true() : json_false());
				if(participant->video)
					json_object_set_new(media, "video_codec", json_string(janus_auviousroom_videocodec_name(participant->vcodec)));
			...
			janus_auviousroom_listener *participant = (janus_auviousroom_listener *)session->participant;
			if(participant) {
				janus_auviousroom_participant *feed = (janus_auviousroom_participant *)participant->feed;
				if(feed) {
					janus_auviousroom *room = feed->room; 
					json_object_set_new(info, "room", room ? json_integer(room->room_id) : NULL);
					json_object_set_new(info, "private_id", json_integer(participant->pvt_id));
...
static int janus_auviousroom_access_room(json_t *root, gboolean check_secret, gboolean check_pin, janus_auviousroom **auviousroom, char *error_cause, int error_cause_size) {
	/* rooms_mutex has to be locked */
	int error_code = 0;
	json_t *room = json_object_get(root, "room");
	guint64 room_id = json_integer_value(room);
	*auviousroom = g_hash_table_lookup(rooms, &room_id);
	if(*auviousroom == NULL) {
		JANUS_LOG(LOG_ERR, "No such room (%"SCNu64")\n", room_id);
		error_code = JANUS_AUVIOUSROOM_ERROR_NO_SUCH_ROOM;
		if(error_cause)
			g_snprintf(error_cause, error_cause_size, "No such room (%"SCNu64")", room_id);
		return error_code;
	}
	if((*auviousroom)->destroyed) {
		JANUS_LOG(LOG_ERR, "No such room (%"SCNu64")\n", room_id);
		error_code = JANUS_AUVIOUSROOM_ERROR_NO_SUCH_ROOM;
		if(error_cause)
			g_snprintf(error_cause, error_cause_size, "No such room (%"SCNu64")", room_id);
	...
	if(check_secret) {
		...
		JANUS_CHECK_SECRET((*auviousroom)->room_secret, root, "secret", error_code, error_cause2,
			JANUS_AUVIOUSROOM_ERROR_MISSING_ELEMENT, JANUS_AUVIOUSROOM_ERROR_INVALID_ELEMENT, JANUS_AUVIOUSROOM_ERROR_UNAUTHORIZED);
		if(error_code != 0) {
	...
	if(check_pin) {
		char error_cause2[100];
		JANUS_CHECK_SECRET((*auviousroom)->room_pin, root, "pin", error_code, error_cause2,
			JANUS_AUVIOUSROOM_ERROR_MISSING_ELEMENT, JANUS_AUVIOUSROOM_ERROR_INVALID_ELEMENT, JANUS_AUVIOUSROOM_ERROR_UNAUTHORIZED);
		if(error_code != 0) {
			g_strlcpy(error_cause, error_cause2, error_cause_size);
			return error_code;
...
struct janus_plugin_result *janus_auviousroom_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep) {
	...
	if(message == NULL) {
		...
		error_code = JANUS_AUVIOUSROOM_ERROR_NO_MESSAGE;
	...
	if(!session) {
		JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
		error_code = JANUS_AUVIOUSROOM_ERROR_UNKNOWN_ERROR;
	...
	if(!json_is_object(root)) {
		...
		error_code = JANUS_AUVIOUSROOM_ERROR_INVALID_JSON;
	...
	JANUS_VALIDATE_JSON_OBJECT(root, request_parameters,
		...
		JANUS_AUVIOUSROOM_ERROR_MISSING_ELEMENT, JANUS_AUVIOUSROOM_ERROR_INVALID_ELEMENT);
	if(error_code != 0)
		goto plugin_response;
	...
	if(!strcasecmp(request_text, "create")) {
		/* Create a new auviousroom */
		JANUS_LOG(LOG_VERB, "Creating a new auviousroom\n");
			...
			if(codec != NULL) {
				...
				while(codec != NULL) {
					...
					if(strlen(codec) == 0 ||
						...
						error_code = JANUS_AUVIOUSROOM_ERROR_INVALID_ELEMENT;
		...
		if(room_id > 0) {
			...
			if(g_hash_table_lookup(rooms, &room_id) != NULL) {
				...
				error_code = JANUS_AUVIOUSROOM_ERROR_ROOM_EXISTS;
		...
		auviousroom->room_id = room_id;
		char *description = NULL;
		if(desc != NULL && strlen(json_string_value(desc)) > 0) {
			description = g_strdup(json_string_value(desc));
		} else {
			char roomname[255];
			g_snprintf(roomname, 255, "Room %"SCNu64"", auviousroom->room_id);
			description = g_strdup(roomname);
		}
		auviousroom->room_name = description;
		auviousroom->is_private = is_private ? json_is_true(is_private) : FALSE;
		auviousroom->require_pvtid = req_pvtid ? json_is_true(req_pvtid) : FALSE;
		if(secret)
			auviousroom->room_secret = g_strdup(json_string_value(secret));
		if(pin)
			auviousroom->room_pin = g_strdup(json_string_value(pin));
		auviousroom->max_publishers = 3;	/* FIXME How should we choose a default? */
		if(publishers)
			auviousroom->max_publishers = json_integer_value(publishers);
		if(auviousroom->max_publishers < 0)
			auviousroom->max_publishers = 3;	/* FIXME How should we choose a default? */
		auviousroom->bitrate = 0;
		if(bitrate)
			auviousroom->bitrate = json_integer_value(bitrate);
		if(auviousroom->bitrate > 0 && auviousroom->bitrate < 64000)
			auviousroom->bitrate = 64000;	/* Don't go below 64k */
		auviousroom->fir_freq = 0;
		if(fir_freq)
			auviousroom->fir_freq = json_integer_value(fir_freq);
		/* By default, we force Opus as the only audio codec */
		auviousroom->acodec[0] = JANUS_AUVIOUSROOM_OPUS;
		auviousroom->acodec[1] = JANUS_AUVIOUSROOM_NOAUDIO;
		auviousroom->acodec[2] = JANUS_AUVIOUSROOM_NOAUDIO;
		...
		auviousroom->audiolevel_ext = audiolevel_ext ? json_is_true(audiolevel_ext) : TRUE;
		auviousroom->audiolevel_event = audiolevel_event ? json_is_true(audiolevel_event) : FALSE;
		if(auviousroom->audiolevel_event) {
			auviousroom->audio_active_packets = 100;
			if(json_integer_value(audio_active_packets) > 0) {
				auviousroom->audio_active_packets = json_integer_value(audio_active_packets);
			} else {
			...
			if(json_integer_value(audio_level_average) > 0) {
				auviousroom->audio_level_average = json_integer_value(audio_level_average);
			} else {
				JANUS_LOG(LOG_WARN, "Invalid audio_level_average value provided, using default: %d\n", auviousroom->audio_level_average);
			}
		}
		auviousroom->videoorient_ext = videoorient_ext ? json_is_true(videoorient_ext) : TRUE;
		auviousroom->playoutdelay_ext = playoutdelay_ext ? json_is_true(playoutdelay_ext) : TRUE;
		/* By default, the auviousroom plugin does not notify about participants simply joining the room.
		   It only notifies when the participant actually starts publishing media. */
		auviousroom->notify_joining = notify_joining ? json_is_true(notify_joining) : FALSE;
		if(record) {
			auviousroom->record = json_is_true(record);
		}
		if(rec_dir) {
			auviousroom->rec_dir = g_strdup(json_string_value(rec_dir));
		}
		auviousroom->destroyed = 0;
		janus_mutex_init(&auviousroom->participants_mutex);
		auviousroom->participants = g_hash_table_new_full(g_int64_hash, g_int64_equal, (GDestroyNotify)g_free, NULL);
		...
		if(allowed != NULL) {
			...
			if(json_array_size(allowed) > 0) {
				...
				for(i=0; i<json_array_size(allowed); i++) {
					...
					if(!g_hash_table_lookup(auviousroom->allowed, token))
						g_hash_table_insert(auviousroom->allowed, g_strdup(token), GINT_TO_POINTER(TRUE));
				}
			}
			auviousroom->check_tokens = TRUE;
		}
		/* Compute a list of the supported codecs for the summary */
		char audio_codecs[100], video_codecs[100];
		memset(audio_codecs, 0, sizeof(audio_codecs));
		...
		json_object_set_new(response, "auviousroom", json_string("created"));
		json_object_set_new(response, "room", json_integer(auviousroom->room_id));
		...
		/* Edit the properties for an existing auviousroom */
		JANUS_LOG(LOG_VERB, "Attempt to edit the properties of an existing auviousroom room\n");
		...
		janus_mutex_lock(&rooms_mutex);
		janus_auviousroom *auviousroom = NULL;
		error_code = janus_auviousroom_access_room(root, TRUE, FALSE, &auviousroom, error_cause, sizeof(error_cause));
		if(error_code != 0) {
			janus_mutex_unlock(&rooms_mutex);
			goto plugin_response;
		}
		...
		if(desc != NULL && strlen(json_string_value(desc)) > 0) {
			char *old_description = auviousroom->room_name;
			char *new_description = g_strdup(json_string_value(desc));
			auviousroom->room_name = new_description;
			g_free(old_description);
		}
		if(is_private)
			auviousroom->is_private = json_is_true(is_private);
		if(req_pvtid)
			auviousroom->require_pvtid = json_is_true(req_pvtid);
		if(publishers)
			auviousroom->max_publishers = json_integer_value(publishers);
		if(bitrate) {
			auviousroom->bitrate = json_integer_value(bitrate);
		...
		if(secret && strlen(json_string_value(secret)) > 0) {
			char *old_secret = auviousroom->room_secret;
			char *new_secret = g_strdup(json_string_value(secret));
			auviousroom->room_secret = new_secret;
			g_free(old_secret);
		}
		if(pin && strlen(json_string_value(pin)) > 0) {
			char *old_pin = auviousroom->room_pin;
			char *new_pin = g_strdup(json_string_value(pin));
			auviousroom->room_pin = new_pin;
			g_free(old_pin);
		}
		if(save) {
			/* This room is permanent: save to the configuration file too
		...
		/* Send info back */
		response = json_object();
		json_object_set_new(response, "auviousroom", json_string("edited"));
		json_object_set_new(response, "room", json_integer(auviousroom->room_id));
		json_object_set_new(response, "permanent", save ? json_true() : json_false());
		/* Also notify event handlers */
		if(notify_events && gateway->events_is_enabled()) {
		...
		JANUS_LOG(LOG_VERB, "Attempt to destroy an existing auviousroom room\n");
		...
		json_object_set_new(destroyed, "auviousroom", json_string("destroyed"));
		json_object_set_new(destroyed, "room", json_integer(auviousroom->room_id));
		GHashTableIter iter;
		gpointer value;
		/* Remove room lazily*/
		auviousroom->destroyed = janus_get_monotonic_time();
		old_rooms = g_list_append(old_rooms, auviousroom);
		janus_mutex_lock(&auviousroom->participants_mutex);
		g_hash_table_iter_init(&iter, auviousroom->participants);
		while (g_hash_table_iter_next(&iter, NULL, &value)) {
			janus_auviousroom_participant *p = value;
			if(p && p->session) {
				/* Notify the user we're going to destroy the room... */
				int ret = gateway->push_event(p->session->handle, &janus_auviousroom_plugin, NULL, destroyed, NULL);
				JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
		...
		janus_mutex_unlock(&auviousroom->participants_mutex);
		/* Also notify event handlers */
		if(notify_events && gateway->events_is_enabled()) {
			json_t *info = json_object();
		...
		/* Done */
		response = json_object();
		json_object_set_new(response, "auviousroom", json_string("destroyed"));
		...
		while(g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			if(!room->destroyed) {
				...
				char audio_codecs[100];
				memset(audio_codecs, 0, sizeof(audio_codecs));
				g_snprintf(audio_codecs, sizeof(audio_codecs), "%s", janus_auviousroom_audiocodec_name(room->acodec[0]));
				if(room->acodec[1] != JANUS_AUVIOUSROOM_NOAUDIO) {
					g_strlcat(audio_codecs, ",", sizeof(audio_codecs));
					g_strlcat(audio_codecs, janus_auviousroom_audiocodec_name(room->acodec[1]), sizeof(audio_codecs));
				}
				if(room->acodec[2] != JANUS_AUVIOUSROOM_NOAUDIO) {
					g_strlcat(audio_codecs, ",", sizeof(audio_codecs));
					g_strlcat(audio_codecs, janus_auviousroom_audiocodec_name(room->acodec[2]), sizeof(audio_codecs));
				}
				json_object_set_new(rl, "audiocodec", json_string(audio_codecs));
				char video_codecs[100];
				memset(video_codecs, 0, sizeof(video_codecs));
				g_snprintf(video_codecs, sizeof(video_codecs), "%s", janus_auviousroom_videocodec_name(room->vcodec[0]));
				if(room->vcodec[1] != JANUS_AUVIOUSROOM_NOVIDEO) {
					g_strlcat(video_codecs, ",", sizeof(video_codecs));
					g_strlcat(video_codecs, janus_auviousroom_videocodec_name(room->vcodec[1]), sizeof(video_codecs));
				}
				if(room->vcodec[2] != JANUS_AUVIOUSROOM_NOVIDEO) {
					g_strlcat(video_codecs, ",", sizeof(video_codecs));
					g_strlcat(video_codecs, janus_auviousroom_videocodec_name(room->vcodec[2]), sizeof(video_codecs));
				}
		...
		janus_mutex_unlock(&rooms_mutex);
		response = json_object();
		json_object_set_new(response, "auviousroom", json_string("success"));
		...
		if(error_code != 0)
			goto plugin_response;
		janus_mutex_lock(&auviousroom->participants_mutex);
		janus_auviousroom_participant* publisher = g_hash_table_lookup(auviousroom->participants, &publisher_id);
		if(publisher == NULL) {
			janus_mutex_unlock(&auviousroom->participants_mutex);
			JANUS_LOG(LOG_ERR, "No such publisher (%"SCNu64")\n", publisher_id);
			error_code = JANUS_AUVIOUSROOM_ERROR_NO_SUCH_FEED;
		...
		if(audio_port > 0) {
			audio_handle = janus_auviousroom_rtp_forwarder_add_helper(publisher, host, audio_port, audio_pt, audio_ssrc, 0, FALSE, FALSE);
		}
		if(video_port[0] > 0) {
			video_handle[0] = janus_auviousroom_rtp_forwarder_add_helper(publisher, host, video_port[0], video_pt[0], video_ssrc[0], 0, TRUE, FALSE);
		}
		if(video_port[1] > 0) {
			video_handle[1] = janus_auviousroom_rtp_forwarder_add_helper(publisher, host, video_port[1], video_pt[1], video_ssrc[1], 1, TRUE, FALSE);
		}
		if(video_port[2] > 0) {
			video_handle[2] = janus_auviousroom_rtp_forwarder_add_helper(publisher, host, video_port[2], video_pt[2], video_ssrc[2], 2, TRUE, FALSE);
		}
		if(data_port > 0) {
			data_handle = janus_auviousroom_rtp_forwarder_add_helper(publisher, host, data_port, 0, 0, 0, FALSE, TRUE);
		}
		janus_mutex_unlock(&auviousroom->participants_mutex);
		response = json_object();
      ...
      janus_auviousroom_reqpli(publisher, "New RTP forward publisher");
			/*char buf[20];
		...
		json_object_set_new(response, "room", json_integer(room_id));
		json_object_set_new(response, "auviousroom", json_string("rtp_forward"));
		goto plugin_response;
		...
		janus_mutex_unlock(&auviousroom->participants_mutex);
		response = json_object();
		json_object_set_new(response, "auviousroom", json_string("stop_rtp_forward"));
		json_object_set_new(response, "room", json_integer(room_id));
		json_object_set_new(response, "publisher_id", json_integer(publisher_id));
		...
		JANUS_LOG(LOG_VERB, "Attempt to edit the list of allowed participants in an existing auviousroom room\n");
		...
		guint64 room_id = json_integer_value(room);
		janus_mutex_lock(&rooms_mutex);
		janus_auviousroom *auviousroom = g_hash_table_lookup(rooms, &room_id);
		if(auviousroom == NULL) {
			janus_mutex_unlock(&rooms_mutex);
			JANUS_LOG(LOG_ERR, "No such room (%"SCNu64")\n", room_id);
			error_code = JANUS_AUVIOUSROOM_ERROR_NO_SUCH_ROOM;
		...
		JANUS_CHECK_SECRET(auviousroom->room_secret, root, "secret", error_code, error_cause,
			JANUS_AUVIOUSROOM_ERROR_MISSING_ELEMENT, JANUS_AUVIOUSROOM_ERROR_INVALID_ELEMENT, JANUS_AUVIOUSROOM_ERROR_UNAUTHORIZED);
		if(error_code != 0) {
			janus_mutex_unlock(&rooms_mutex);
			goto plugin_response;
		...
		if(!strcasecmp(action_text, "enable")) {
			...
			auviousroom->check_tokens = FALSE;
		} else {
			...
			if(allowed) {
				...
				for(i=0; i<json_array_size(allowed); i++) {
					...
					if(add) {
						...
						g_hash_table_remove(auviousroom->allowed, token);
					}
				}
			}
		}
		...
		if(strcasecmp(action_text, "disable")) {
			if(g_hash_table_size(auviousroom->allowed) > 0) {
				GHashTableIter iter;
				gpointer key;
				g_hash_table_iter_init(&iter, auviousroom->allowed);
		...
		/* Done */
		janus_mutex_unlock(&rooms_mutex);
		JANUS_LOG(LOG_VERB, "AuviousRoom room allowed list updated\n");
		goto plugin_response;
	} else if(!strcasecmp(request_text, "kick")) {
		JANUS_LOG(LOG_VERB, "Attempt to kick a participant from an existing auviousroom room\n");
		...
		janus_auviousroom_participant *participant = g_hash_table_lookup(auviousroom->participants, &user_id);
		if(participant == NULL) {
			janus_mutex_unlock(&auviousroom->participants_mutex);
		...
		json_object_set_new(kicked, "auviousroom", json_string("event"));
		...
		int ret = gateway->push_event(participant->session->handle, &janus_auviousroom_plugin, NULL, kicked, NULL);
		JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
		json_decref(kicked);
		janus_mutex_unlock(&auviousroom->participants_mutex);
		/* If this room requires valid private_id values, we can kick subscriptions too */
		if(auviousroom->require_pvtid && participant->subscriptions != NULL) {
			...
			while(s) {
				janus_auviousroom_listener *listener = (janus_auviousroom_listener *)s->data;
				if(listener) {
		...
		janus_auviousroom_leave_or_unpublish(participant, TRUE, TRUE);
		...
		janus_mutex_lock(&rooms_mutex);
		janus_auviousroom *auviousroom = NULL;
		error_code = janus_auviousroom_access_room(root, FALSE, FALSE, &auviousroom, error_cause, sizeof(error_cause));
		janus_mutex_unlock(&rooms_mutex);
		if(error_code != 0)
			goto plugin_response;
		...
		janus_mutex_lock(&auviousroom->participants_mutex);
		g_hash_table_iter_init(&iter, auviousroom->participants);
		while (!auviousroom->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
			janus_auviousroom_participant *p = value;
		...
		janus_mutex_unlock(&auviousroom->participants_mutex);
		response = json_object();
		json_object_set_new(response, "auviousroom", json_string("participants"));
		json_object_set_new(response, "room", json_integer(room_id));
		...
		while (!auviousroom->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			while(g_hash_table_iter_next(&iter_f, &key_f, &value_f)) {				
				...
				janus_auviousroom_rtp_forwarder *rpv = value_f;
		...
		janus_auviousroom_message *msg = g_malloc0(sizeof(janus_auviousroom_message));
		msg->handle = handle;
		...
		error_code = JANUS_AUVIOUSROOM_ERROR_INVALID_REQUEST;
...
void janus_auviousroom_setup_media(janus_plugin_session *handle) {
	...
	if(session->participant) {
		...
		if(session->participant_type == janus_auviousroom_p_type_publisher) {
			...
			if(participant->audio)
				json_object_set_new(pl, "audio_codec", json_string(janus_auviousroom_audiocodec_name(participant->acodec)));
			if(participant->video)
				json_object_set_new(pl, "video_codec", json_string(janus_auviousroom_videocodec_name(participant->vcodec)));
			json_array_append_new(list, pl);
			json_t *pub = json_object();
			json_object_set_new(pub, "auviousroom", json_string("event"));
			...
			while (!auviousroom->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
				...
				JANUS_LOG(LOG_VERB, "Notifying participant %"SCNu64" (%s)\n", p->user_id, p->display ? p->display : "??");
				int ret = gateway->push_event(p->session->handle, &janus_auviousroom_plugin, NULL, pub, NULL);
				JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
			}
			json_decref(pub);
			janus_mutex_unlock(&auviousroom->participants_mutex);
			...
			janus_auviousroom_listener *l = (janus_auviousroom_listener *)session->participant;
			if(l && l->feed) {
				janus_auviousroom_participant *p = l->feed;
				if(p && p->session) {
          janus_auviousroom_reqpli(p, "New listener available");
					/* Send a FIR */
					/*char buf[20];
...
void janus_auviousroom_incoming_rtp(janus_plugin_session *handle, janus_plugin_rtp *pkt) {
	if(handle == NULL || handle->stopped || g_atomic_int_get(&stopping) || !g_atomic_int_get(&initialized) || !gateway)
		return;
	janus_auviousroom_session *session = (janus_auviousroom_session *)handle->plugin_handle;
	if(!session || session->destroyed || !session->participant || session->participant_type != janus_auviousroom_p_type_publisher)
		return;
	janus_auviousroom_participant *participant = (janus_auviousroom_participant *)session->participant;
	janus_auviousroom *auviousroom = participant->room;
  ...
  uint16_t len = pkt->length;
	...
	if(!video && auviousroom->audiolevel_event && participant->audio_active) {
		...
		if(level != -1) {
			...
			if(participant->audio_active_packets > 0 && participant->audio_active_packets == auviousroom->audio_active_packets) {
				gboolean notify_talk_event = FALSE;
				if((float)participant->audio_dBov_sum/(float)participant->audio_active_packets < auviousroom->audio_level_average) {
				...
				if(notify_talk_event) {
					janus_mutex_lock(&participant->room->participants_mutex);
					json_t *event = json_object();
					json_object_set_new(event, "auviousroom", json_string(participant->talking ? "talking" : "stopped-talking"));
					json_object_set_new(event, "room", json_integer(participant->room->room_id));
					...
					if(notify_events && gateway->events_is_enabled()) {
						json_t *info = json_object();
						json_object_set_new(info, "auviousroom", json_string(participant->talking ? "talking" : "stopped-talking"));
						json_object_set_new(info, "room", json_integer(participant->room->room_id));
						json_object_set_new(info, "id", json_integer(participant->user_id));
						gateway->notify_event(&janus_auviousroom_plugin, session->handle, info);
					}
	...
	if((!video && participant->audio_active) || (video && participant->video_active)) {
		...
		while(participant->udp_sock > 0 && g_hash_table_iter_next(&iter, NULL, &value)) {
			janus_auviousroom_rtp_forwarder* rtp_forward = (janus_auviousroom_rtp_forwarder*)value;
		...
		/* Done, relay it */
		janus_auviousroom_rtp_relay_packet packet;
		packet.data = rtp;
		packet.length = len;
    packet.extensions = pkt->extensions;
		packet.is_video = video;
		packet.svc = FALSE;
		if(video && auviousroom->do_svc) {
      ...
      if(janus_vp9_parse_svc(payload, plen, &found, &packet.svc_info) == 0) {
		...
		g_slist_foreach(participant->listeners, janus_auviousroom_relay_rtp_packet, &packet);
        ...
        gateway->send_remb(handle, bitrate);
          ...
          janus_auviousroom_reqpli(participant, "Regular keyframe sent");
          /*
					char rtcpbuf[24];
	...
	if(session->participant_type == janus_auviousroom_p_type_subscriber) {
		...
		if(janus_rtcp_has_fir(buf, len) || janus_rtcp_has_pli(buf, len)) {
			...
			if(l->feed) {
				janus_auviousroom_participant *p = l->feed;
				if(p && p->session) {
          janus_auviousroom_reqpli(p, "PLI from subscriber");
				}
			}
		}
...
void janus_auviousroom_incoming_data(janus_plugin_session *handle, janus_plugin_data *packet) {
  ...
  uint16_t len = packet->length;
	...
	*(text+len) = '\0';
	...
	janus_mutex_lock_nodebug(&participant->listeners_mutex);
	g_slist_foreach(participant->listeners, janus_auviousroom_relay_data_packet, text);
	janus_mutex_unlock_nodebug(&participant->listeners_mutex);
	g_free(text);
}
...
void janus_auviousroom_slow_link(janus_plugin_session *handle, int uplink, int video) {
	...
	if(session->participant_type == janus_auviousroom_p_type_publisher) {
		if(!uplink) {
			janus_auviousroom_participant *publisher = (janus_auviousroom_participant *)session->participant;
			if(publisher) {
				/* Send an event on the handle to notify the application: it's
				 * up to the application to then choose a policy and enforce it */
				json_t *event = json_object();
				json_object_set_new(event, "auviousroom", json_string("slow_link"));
				...
				gateway->push_event(session->handle, &janus_auviousroom_plugin, NULL, event, NULL);
				json_decref(event);
			}
		} else {
			JANUS_LOG(LOG_WARN, "Got a slow uplink on a AuviousRoom publisher? Weird, because it doesn't receive media...\n");
		}
	} else if(session->participant_type == janus_auviousroom_p_type_subscriber) {
		if(uplink) {
			janus_auviousroom_listener *viewer = (janus_auviousroom_listener *)session->participant;
			if(viewer) {
				/* Send an event on the handle to notify the application: it's
				 * up to the application to then choose a policy and enforce it */
				json_t *event = json_object();
			...
			JANUS_LOG(LOG_WARN, "Got a slow downlink on a AuviousRoom viewer? Weird, because it doesn't send media...\n");
		}
	}
}

static void janus_auviousroom_recorder_create(janus_auviousroom_participant *participant, gboolean audio, gboolean video, gboolean data) {
	char filename[255];
	...
	if(audio) {
		...
		if(participant->recording_base) {
			...
			participant->arc = janus_recorder_create(participant->room->rec_dir,
				janus_auviousroom_audiocodec_name(participant->acodec), filename);
			if(participant->arc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an audio recording file for this publisher!\n");
			}
		} else {
			/* Build a filename */
			g_snprintf(filename, 255, "auviousroom-%"SCNu64"-user-%"SCNu64"-%"SCNi64"-audio",
	...
	if(video) {
		...
		if(participant->recording_base) {
			...
			participant->vrc = janus_recorder_create(participant->room->rec_dir,
				janus_auviousroom_videocodec_name(participant->vcodec), filename);
			if(participant->vrc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an video recording file for this publisher!\n");
			}
		} else {
			/* Build a filename */
			g_snprintf(filename, 255, "auviousroom-%"SCNu64"-user-%"SCNu64"-%"SCNi64"-video",
				participant->room->room_id, participant->user_id, now);
			participant->vrc = janus_recorder_create(participant->room->rec_dir,
				janus_auviousroom_videocodec_name(participant->vcodec), filename);
			if(participant->vrc == NULL) {
	...
	if(data) {
		...
		if(participant->recording_base) {
			...
			/* Build a filename */
			g_snprintf(filename, 255, "auviousroom-%"SCNu64"-user-%"SCNu64"-%"SCNi64"-data",
				participant->room->room_id, participant->user_id, now);
			participant->drc = janus_recorder_create(participant->room->rec_dir,
				"text", filename);
			if(participant->drc == NULL) {
...
static void janus_auviousroom_recorder_close(janus_auviousroom_participant *participant) {
	if(participant->arc) {
...
void janus_auviousroom_hangup_media(janus_plugin_session *handle) {
	...
	if(session->participant_type == janus_auviousroom_p_type_publisher) {
		...
		janus_auviousroom_recorder_close(participant);
		janus_mutex_unlock(&participant->rec_mutex);
		janus_mutex_lock(&participant->listeners_mutex);
		while(participant->listeners) {
			janus_auviousroom_listener *l = (janus_auviousroom_listener *)participant->listeners->data;
			if(l) {
		...
		janus_mutex_unlock(&participant->listeners_mutex);
		janus_auviousroom_leave_or_unpublish(participant, FALSE, FALSE);
		/* Also notify event handlers */
		if(participant->room && gateway->events_is_enabled()) {
			...
			json_object_set_new(info, "room", json_integer(participant->room->room_id));
			json_object_set_new(info, "id", json_integer(participant->user_id));
			gateway->notify_event(&janus_auviousroom_plugin, handle, info);
		}
	} else if(session->participant_type == janus_auviousroom_p_type_subscriber) {
		/* Get rid of listener */
		...
		if(listener) {
			...
			if(publisher != NULL) {
				...
				if(listener->pvt_id > 0) {
					janus_auviousroom_participant *owner = g_hash_table_lookup(publisher->room->private_ids, GUINT_TO_POINTER(listener->pvt_id));
					if(owner != NULL) {
...
static void *janus_auviousroom_handler(void *data) {
	JANUS_LOG(LOG_VERB, "Joining AuviousRoom handler thread\n");
	janus_auviousroom_message *msg = NULL;
	int error_code = 0;
	char error_cause[512];
	json_t *root = NULL;
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		janus_auviousroom_session *session = NULL;
		janus_mutex_lock(&sessions_mutex);
		if(g_hash_table_lookup(sessions, msg->handle) != NULL ) {
			session = (janus_auviousroom_session *)msg->handle->plugin_handle;
		}
		if(!session) {
			janus_mutex_unlock(&sessions_mutex);
			JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
		...
		if(session->participant_type == janus_auviousroom_p_type_none) {
			...
			if(strcasecmp(request_text, "join") && strcasecmp(request_text, "joinandconfigure")) {
				...
				error_code = JANUS_AUVIOUSROOM_ERROR_JOIN_FIRST;
			...
			janus_mutex_lock(&rooms_mutex);
			janus_auviousroom *auviousroom = NULL;
			error_code = janus_auviousroom_access_room(root, FALSE, TRUE, &auviousroom, error_cause, sizeof(error_cause));
			janus_mutex_unlock(&rooms_mutex);
			if(error_code != 0)
				goto error;
			...
			if(!strcasecmp(ptype_text, "publisher")) {
				...
				if(auviousroom->check_tokens) {
					json_t *token = json_object_get(root, "token");
					const char *token_text = token ? json_string_value(token) : NULL;
					if(token_text == NULL || g_hash_table_lookup(auviousroom->allowed, token_text) == NULL) {
						JANUS_LOG(LOG_ERR, "Unauthorized (not in the allowed list)\n");
						error_code = JANUS_AUVIOUSROOM_ERROR_UNAUTHORIZED;
				...
				if(id) {
					...
					if(g_hash_table_lookup(auviousroom->participants, &user_id) != NULL) {
						janus_mutex_unlock(&auviousroom->participants_mutex);
						/* User ID already taken */
						JANUS_LOG(LOG_ERR, "User ID %"SCNu64" already exists\n", user_id);
						error_code = JANUS_AUVIOUSROOM_ERROR_ID_EXISTS;
				...
				janus_auviousroom_participant *publisher = g_malloc0(sizeof(janus_auviousroom_participant));
				publisher->session = session;
				publisher->room = auviousroom;
				...
				publisher->acodec = JANUS_AUVIOUSROOM_NOAUDIO;	/* We'll deal with this later */
				publisher->vcodec = JANUS_AUVIOUSROOM_NOVIDEO;	/* We'll deal with this later */
				...
				publisher->bitrate = auviousroom->bitrate;
				...
				publisher->rtp_forwarders = g_hash_table_new_full(NULL, NULL, NULL, (GDestroyNotify)janus_auviousroom_rtp_forwarder_free_helper);
				...
				while(publisher->pvt_id == 0) {
					...
					if(g_hash_table_lookup(auviousroom->private_ids, GUINT_TO_POINTER(publisher->pvt_id)) != NULL) {
						/* Private ID already taken, try another one */
						publisher->pvt_id = 0;
					}
					g_hash_table_insert(auviousroom->private_ids, GUINT_TO_POINTER(publisher->pvt_id), publisher);
				}
				...
				/* Done */
				session->participant_type = janus_auviousroom_p_type_publisher;
				session->participant = publisher;
				/* Return a list of all available publishers (those with an SDP available, that is) */
				json_t *list = json_array();
				GHashTableIter iter;
				gpointer value;
				g_hash_table_insert(auviousroom->participants, janus_uint64_dup(publisher->user_id), publisher);
				g_hash_table_iter_init(&iter, auviousroom->participants);
				while (!auviousroom->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
					janus_auviousroom_participant *p = value;
					...
					if(p->display)
						json_object_set_new(pl, "display", json_string(p->display));
					if(p->audio)
						json_object_set_new(pl, "audio_codec", json_string(janus_auviousroom_audiocodec_name(p->acodec)));
					if(p->video)
						json_object_set_new(pl, "video_codec", json_string(janus_auviousroom_videocodec_name(p->vcodec)));
					if(p->audio_level_extmap_id > 0)
						json_object_set_new(pl, "talking", p->talking ? json_true() : json_false());
					json_array_append_new(list, pl);
				}
				janus_mutex_unlock(&auviousroom->participants_mutex);
				event = json_object();
				json_object_set_new(event, "auviousroom", json_string("joined"));
				json_object_set_new(event, "room", json_integer(auviousroom->room_id));
				json_object_set_new(event, "description", json_string(auviousroom->room_name));
				...
				janus_auviousroom_participant_joining(publisher);
				...
				if(publisher == NULL || publisher->sdp == NULL) {
					...
					if(auviousroom->require_pvtid) {
						...
						owner = g_hash_table_lookup(auviousroom->private_ids, GUINT_TO_POINTER(pvt_id));
					...
					janus_auviousroom_listener *listener = g_malloc0(sizeof(janus_auviousroom_listener));
					listener->session = session;
					listener->room = auviousroom;
					...
					if((!publisher->audio || !listener->audio_offered) &&
						...
						error_code = JANUS_AUVIOUSROOM_ERROR_INVALID_SDP;
					...
					event = json_object();
					json_object_set_new(event, "auviousroom", json_string("attached"));
					json_object_set_new(event, "room", json_integer(auviousroom->room_id));
					...
					if(publisher->sdp != NULL) {
						...
						g_atomic_int_set(&session->hangingup, 0);
						gint64 start = janus_get_monotonic_time();
						int res = gateway->push_event(msg->handle, &janus_auviousroom_plugin, msg->transaction, event, jsep);
						JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (took %"SCNu64" us)\n", res, janus_get_monotonic_time()-start);
						json_decref(event);
						json_decref(jsep);
						janus_auviousroom_message_free(msg);
			...
			if(!strcasecmp(request_text, "join") || !strcasecmp(request_text, "joinandconfigure")) {
				...
				error_code = JANUS_AUVIOUSROOM_ERROR_ALREADY_JOINED;
				...
				if(!strcasecmp(request_text, "publish") && participant->sdp) {
					...
					error_code = JANUS_AUVIOUSROOM_ERROR_ALREADY_PUBLISHED;
				...
				if(audio) {
					...
					if(session->started && audio_active && !participant->audio_active) {
						...
						while(ps) {
							janus_auviousroom_listener *l = (janus_auviousroom_listener *)ps->data;
							if(l)
				...
				if(audiocodec && json_string_value(json_object_get(msg->jsep, "sdp")) != NULL) {
					...
					janus_auviousroom_audiocodec acodec = janus_auviousroom_audiocodec_from_name(json_string_value(audiocodec));
					if(acodec == JANUS_AUVIOUSROOM_NOAUDIO ||
				...
				if(videocodec && json_string_value(json_object_get(msg->jsep, "sdp")) != NULL) {
					...
					janus_auviousroom_videocodec vcodec = janus_auviousroom_videocodec_from_name(json_string_value(videocodec));
					if(vcodec == JANUS_AUVIOUSROOM_NOVIDEO ||
				...
				if(bitrate) {
					...
					if(g_atomic_int_get(&session->started))
						participant->remb_latest = janus_get_monotonic_time();
          gateway->send_remb(msg->handle, participant->bitrate ? participant->bitrate : 256*1024);
				}
				janus_mutex_lock(&participant->rec_mutex);
				...
				if(participant->recording_active != prev_recording_active) {
					...
					if(!participant->recording_active) {
						...
						janus_auviousroom_recorder_create(
              ...
              janus_auviousroom_reqpli(participant, "Recording video");
						}
					}
				}
				janus_mutex_unlock(&participant->rec_mutex);
				if(display) {
					...
					json_object_set_new(display_event, "auviousroom", json_string("event"));
					...
					if(participant->room && !participant->room->destroyed) {
						janus_auviousroom_notify_participants(participant, display_event);
					}
					janus_mutex_unlock(&participant->room->participants_mutex);
				...
				if(!participant->sdp) {
					...
					error_code = JANUS_AUVIOUSROOM_ERROR_NOT_PUBLISHED;
				...
				if(publisher) {
					...
					if(sc_substream && publisher->ssrc[0] != 0) {
						...
						if(listener->substream_target == listener->substream) {
							...
							gateway->push_event(msg->handle, &janus_auviousroom_plugin, NULL, event, NULL);
							json_decref(event);
						} else {
							/* Send a FIR */
              janus_auviousroom_reqpli(publisher, "Simulcasting substream change");
						}
					}
					if(sc_temporal && publisher->ssrc[0] != 0) {
						...
						if(listener->templayer_target == listener->templayer) {
							...
							/* Send a FIR */
              janus_auviousroom_reqpli(publisher, "Simulcasting temporal layer change");
						}
					}
				}
				if(listener->room->do_svc) {
					...
					if(spatial) {
						...
						if(spatial_layer == listener->spatial_layer) {
							...
							/* Send a FIR to the new RTP forward publisher */
              janus_auviousroom_reqpli(publisher, "Need to downscale spatially");
						}
				...
				janus_auviousroom_participant *publisher = g_hash_table_lookup(listener->room->participants, &feed_id);
				...
				janus_auviousroom_participant *prev_feed = listener->feed;
				if(prev_feed) {
        ...
        janus_auviousroom_reqpli(publisher, "Switching existing subscriber to new publisher");
				/* Done */
				listener->paused = paused;
				event = json_object();
				json_object_set_new(event, "auviousroom", json_string("event"));
				...
				if(listener->pvt_id > 0) {
					janus_auviousroom_participant *owner = g_hash_table_lookup(listener->room->private_ids, GUINT_TO_POINTER(listener->pvt_id));
					if(owner != NULL) {
						janus_mutex_lock(&owner->listeners_mutex);
						owner->subscriptions = g_slist_remove(owner->subscriptions, listener);
						janus_mutex_unlock(&owner->listeners_mutex);
		...
		if(!msg_sdp) {
			int ret = gateway->push_event(msg->handle, &janus_auviousroom_plugin, msg->transaction, event, NULL);
			JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
			json_decref(event);
		} else {
			...
			if(!strcasecmp(msg_sdp_type, "offer")) {
				...
				error_code = JANUS_AUVIOUSROOM_ERROR_INVALID_SDP_TYPE;
			...
			if(session->participant_type == janus_auviousroom_p_type_subscriber) {
				...
				if(listener->firefox) {	/* Don't add any b=AS attribute for Chrome */
					...
					if(m != NULL && auviousroom->bitrate > 0) {
						...
						m->b_value = (int)(auviousroom->bitrate/1000);
					}
				}
				...
				janus_mutex_unlock(&auviousroom->participants_mutex);
				if(count == auviousroom->max_publishers) {
					participant->audio_active = FALSE;
					participant->video_active = FALSE;
					participant->data_active = FALSE;
					JANUS_LOG(LOG_ERR, "Maximum number of publishers (%d) already reached\n", auviousroom->max_publishers);
					error_code = JANUS_AUVIOUSROOM_ERROR_PUBLISHERS_FULL;
					g_snprintf(error_cause, 512, "Maximum number of publishers (%d) already reached", auviousroom->max_publishers);
					goto error;
				}
				/* Now prepare the SDP to give back */
				if(strstr(msg_sdp, "Mozilla")) {
				...
				offer = janus_sdp_generate_offer(s_name, answer->c_addr,
					...
					JANUS_SDP_OA_AUDIO_CODEC, janus_auviousroom_audiocodec_name(participant->acodec),
					JANUS_SDP_OA_AUDIO_PT, janus_auviousroom_audiocodec_pt(participant->acodec),
					JANUS_SDP_OA_AUDIO_DIRECTION, JANUS_SDP_SENDONLY,
					JANUS_SDP_OA_VIDEO, participant->video,
					JANUS_SDP_OA_VIDEO_CODEC, janus_auviousroom_videocodec_name(participant->vcodec),
					JANUS_SDP_OA_VIDEO_PT, janus_auviousroom_videocodec_pt(participant->vcodec),
					JANUS_SDP_OA_VIDEO_DIRECTION, JANUS_SDP_SENDONLY,
				...
				if(auviousroom->record || participant->recording_active) {
					janus_auviousroom_recorder_create(participant, participant->audio, participant->video, participant->data);
				}
				/* Is simulcasting involved */
				if(msg_simulcast && participant->vcodec == JANUS_AUVIOUSROOM_VP8) {
...
error:
	...
	JANUS_LOG(LOG_VERB, "Leaving AuviousRoom handler thread\n");
	return NULL;
}

/* Helper to quickly relay RTP packets from publishers to subscribers */
static void janus_auviousroom_relay_rtp_packet(gpointer data, gpointer user_data) {
	janus_auviousroom_rtp_relay_packet *packet = (janus_auviousroom_rtp_relay_packet *)user_data;
	if(!packet || !packet->data || packet->length < 1) {
		JANUS_LOG(LOG_ERR, "Invalid packet...\n");
		return;
	}
	janus_auviousroom_listener *listener = (janus_auviousroom_listener *)data;
	...
	janus_auviousroom_session *session = listener->session;
	...
	if(packet->is_video) {
		...
		if(packet->svc) {
			...
			if(listener->target_temporal_layer > listener->temporal_layer) {
				...
				if(packet->svc_info.ubit && packet->svc_info.bbit && packet->svc_info.temporal_layer <= listener->target_temporal_layer) {
					...
					json_object_set_new(event, "room", json_integer(listener->room->room_id));
					json_object_set_new(event, "temporal_layer", json_integer(listener->temporal_layer));
					gateway->push_event(listener->session->handle, &janus_auviousroom_plugin, NULL, event, NULL);
					json_decref(event);
				}
        ...
        janus_plugin_rtp rtp = { .video = packet->is_video, .buffer = (char *)packet->data, .length = packet->length, .extensions = packet->extensions };
			...
			if(listener->last_relayed == 0) {
              ...
              gateway->send_pli(listener->feed->session->handle);
						/* Notify the viewer */
						json_t *event = json_object();
						json_object_set_new(event, "auviousroom", json_string("event"));
						json_object_set_new(event, "room", json_integer(listener->room->room_id));
...
/* Helper to free janus_auviousroom structs. */
static void janus_auviousroom_free(janus_auviousroom *room) {
	if(room) {
...
static void janus_auviousroom_participant_free(janus_auviousroom_participant *p) {
	...
	while(p->listeners) {
		janus_auviousroom_listener *l = (janus_auviousroom_listener *)p->listeners->data;
		if(l) {
