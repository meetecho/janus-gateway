...
void janus_textroom_incoming_data(janus_plugin_session *handle, char *label, char *protocol, gboolean textdata, char *buf, int len);
	...
	if(!strcasecmp(request_text, "message")) {
		...
		if(username) {
			...
			if(top) {
				...
				gateway->relay_data(top->session->handle, NULL, NULL, TRUE, msg_text, strlen(msg_text));
		...
		if(textroom->participants) {
			...
			json_decref(event);
			gateway->relay_data(handle, NULL, NULL, TRUE, event_text, strlen(event_text));
			/* Broadcast */
			GHashTableIter iter;
			gpointer value;
			g_hash_table_iter_init(&iter, textroom->participants);
			while(g_hash_table_iter_next(&iter, NULL, &value)) {
				...
				gateway->relay_data(top->session->handle, NULL, NULL, TRUE, event_text, strlen(event_text));
...
msg_response:
		{
			if(!internal) {
				...
				if(json == NULL) {
					...
					json_decref(reply);
					gateway->relay_data(handle, NULL, NULL, TRUE, reply_text, strlen(reply_text));
					free(reply_text);
				} else {
					/* Reply via Janus API */
