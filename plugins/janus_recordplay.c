	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(!strcasecmp(request_text, "record")) {
			...
			if(audio) {
				...
				rc = janus_recorder_create(recordings_path, janus_audiocodec_name(rec->acodec), rec->arc_file);
				if(perc)
			...
			if(video) {
				...
				rc = janus_recorder_create(recordings_path, janus_videocodec_name(rec->vcodec), rec->vrc_file);
				if(perc)
					janus_recorder_enable_perc(rc);
				session->vrc = rc;
			}
