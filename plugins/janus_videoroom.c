			...
			if(p && s->autochange && (p->simulcast || p->vp9svc)) {
				...
				if(p) {
					...
					if(tl < 0)
						tl = 0;
					if(p->simulcast) {
						if(sl != s->sim_context.substream_target || tl != s->sim_context.templayer_target) {
							JANUS_LOG(LOG_WARN, "[%p] Autochanging to substream #%d (was #%d), temporal layer %d (was #%d): %"SCNu32" < %"SCNu32"\n",
								s, sl, s->sim_context.substream_target, tl, s->sim_context.templayer_target, p->tlrates[sl][tl], bitrate);
							s->sim_context.substream_target = sl;
							s->sim_context.templayer_target = tl;
						}
					} else if(p->vp9svc) {
						if(sl != s->target_spatial_layer || tl != s->target_temporal_layer) {
							JANUS_LOG(LOG_WARN, "[%p] Autochanging to spatial layer #%d (was #%d), temporal layer %d (was #%d): %"SCNu32" < %"SCNu32"\n",
								s, sl, s->target_spatial_layer, tl, s->target_temporal_layer, p->tlrates[sl][tl], bitrate);
		...
		if(session->participant_type == janus_videoroom_p_type_none) {
			...
			if(!strcasecmp(request_text, "join")) {
				...
				if(publisher) {
					...
					if(sc_temporal && publisher->simulcast && publisher->vcodec == JANUS_VIDEOCODEC_VP8) {
