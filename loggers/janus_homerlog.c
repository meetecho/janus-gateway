 ...
 * \brief  Janus Homer logger plugin (via HEP)
 * \details  This is a Homer logger plugin for Janus, which uses the HEP
 * protocol (documented on the http://hep.sipcapture.org/ website) to
 * send Janus log lines to a remote Homer instance. This is particularly
 * useful to store and go through logs using Homer, and doubly so if
 * Homer is also used to collect Janus events via event handlers.
 *
 * \ingroup loggers
 * \ref loggers
 */
...
#define JANUS_HOMERLOG_DESCRIPTION		"This is a trivial sample logger plugin for Janus, which saves log lines to a local JSON file."
#define JANUS_HOMERLOG_NAME				"JANUS Homer logger (HEP) plugin"
#define JANUS_HOMERLOG_AUTHOR			"Meetecho s.r.l."
#define JANUS_HOMERLOG_PACKAGE			"janus.logger.homerlog"

/* Plugin methods */
janus_logger *create(void);
int janus_homerlog_init(const char *config_path);
...
static uint16_t homer_port = 9060;
static gboolean homer_udp = FALSE;
...
int janus_homerlog_init(const char *config_path) {
	...
	if(config != NULL) {
		...
		if(!item || !item->value || !janus_is_true(item->value)) {
			JANUS_LOG(LOG_WARN, "Homer logger (HEP) disabled\n");
			goto done;
		}
		...
		item = janus_config_get(config, config_general, janus_config_type_item, "address_protocol");
		if(item && item->value) {
			if(!strcasecmp(item->value, "udp")) {
				homer_udp = TRUE;
			} else if(!strcasecmp(item->value, "tcp")) {
				/* Use plain TCP */
				homer_udp = FALSE;
			} else {
				JANUS_LOG(LOG_ERR, "Unsupported protocol scheme '%s'...\n", item->value);
				goto done;
			}
		} else {
			JANUS_LOG(LOG_WARN, "HEP transport not specified, assuming UDP...\n");
			homer_udp = TRUE;
		}
		item = janus_config_get(config, config_general, janus_config_type_item, "address_port");
		if(item && item->value) {
			uint16_t port = atoi(item->value);
			if(port > 0) {
				homer_port = port;
			} else {
				JANUS_LOG(LOG_WARN, "HEP port invalid, falling back to %"SCNu16"...\n", homer_port);
			}
		} else {
			JANUS_LOG(LOG_WARN, "HEP port not specified, falling back to %"SCNu16"...\n", homer_port);
		}
		item = janus_config_get(config, config_general, janus_config_type_item, "address_host");
		if(item == NULL || item->value == NULL) {
			JANUS_LOG(LOG_ERR, "HEP address not specified...\n");
			goto done;
		}
		...
		if(getaddrinfo(item->value, NULL, NULL, &res) == 0) {
			start = res;
			while(res != NULL) {
		...
		homer_address = g_strdup(resolved_host);

		/* Create the socket and connect */
		JANUS_LOG(LOG_INFO, "Connecting to Homer: %s://%s:%"SCNu16" (capture agent ID: %"SCNu32")\n",
			homer_udp ? "udp" : "tcp", homer_address, homer_port, homer_agent_id);
		homer_fd = socket(AF_INET6, homer_udp ? SOCK_DGRAM : SOCK_STREAM, homer_udp ? IPPROTO_UDP : IPPROTO_TCP);
		int v6only = 0;
		if(homer_fd <= 0 ||
			...
			JANUS_LOG(LOG_ERR, "Error creating Homer HEP %s socket... %d (%s)\n",
				homer_udp ? "UDP" : "TCP", errno, strerror(errno));
			goto done;
		}
	...
	JANUS_LOG(LOG_VERB, "Homer logger (HEP) configured: %d\n", homer_fd);
	...
	if(error != NULL) {
		...
		JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the Homer logger (HEP) thread...\n",
...
static void *janus_homerlog_thread(void *data) {
	JANUS_LOG(LOG_VERB, "Joining Homer logger (HEP) thread\n");
	...
	JANUS_LOG(LOG_VERB, "Leaving Homer logger (HEP) thread\n");
	return NULL;
}
