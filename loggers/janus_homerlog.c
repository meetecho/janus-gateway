...
int janus_homerlog_init(const char *server_name, const char *config_path) {
...
#else
	JANUS_LOG(LOG_WARN, "libcurl not available, HTTP logging to HOMER won't be available\n");
#endif
	...
	if(config != NULL) {
		...
		} else if(strstr(homer_address, "http://") == homer_address || strstr(homer_address, "https://") == homer_address) {
#ifdef HAVE_LIBCURL
			/* HTTP: we can use libcurl */
			homer_transport = JANUS_HOMERLOG_HEP3_CURL;
#else
			JANUS_LOG(LOG_ERR, "libcurl not available, unsupported address scheme '%s'...\n", homer_address);
			goto done;
#endif
		}
